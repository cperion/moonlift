# Moonlift Debugger — Architecture Exploration 
Design a debugger for Moonlift that leverages explicit typed block/jump control flow, named transitions, and ASDL-backed compiler metadata to provide a fundamentally new debugging experience — state machine navigation, transition inspection, semantic breakpoints, and reverse-step through predecessor chains.
...
1. **`lua/moonlift/schema/tree.lua`** (full, 588 lines) — The MoonTree ASDL schema. Defines the complete AST: `ControlStmtRegion`, `ControlExprRegion`, `EntryControlBlock`, `ControlBlock`, `BlockLabel`, `BlockParam`, `EntryBlockParam`, `JumpArg`, `StmtJump`, `StmtYieldVoid`, `StmtYieldValue`, `StmtReturnVoid`, `StmtReturnValue`, `StmtTrap`, `StmtSwitch`. Also defines `ControlFact` enum with detailed facts: `ControlFactEntryBlock`, `ControlFactBlock`, `ControlFactJump`, `ControlFactJumpArg`, `ControlFactYieldVoid`, `ControlFactYieldValue`, `ControlFactReturn`, `ControlFactBackedge`, `ControlFactVariantSwitch`, `ControlFactEntryParam`, `ControlFactBlockParam`.

2. **`lua/moonlift/schema/sem.lua`** (full, 101 lines) — MoonSem schema. Defines `FieldRef`, `FlowClass` (FlowUnknown, FlowFallsThrough, FlowJumps, FlowYields, FlowReturns, FlowTerminates), `ConstValue`, `TypeLayout`, `LayoutEnv`.

3. **`lua/moonlift/schema/back.lua`** (full, 670 lines) — MoonBack schema. The flat backend command language: `CmdCreateBlock`, `CmdSwitchToBlock`, `CmdSealBlock`, `CmdAppendBlockParam`, `CmdBindEntryParams`, `CmdJump`, `CmdBrIf`, `CmdSwitchInt`, `CmdReturnVoid`, `CmdReturnValue`, `CmdTrap`, `CmdBeginFunc`, `CmdFinishFunc`, `CmdFinalizeModule`. Also defines `BackFlow` (FallsThrough/Terminates), `BackProvenance` types, `BackValidationIssue`, `BackInspectionReport`, `BackDiagnosticsReport`.

4. **`lua/moonlift/schema/source.lua`** (full, 243 lines) — MoonSource schema. All source tracking types: `DocUri`, `DocumentSnapshot`, `SourcePos`, `SourceRange`, `AnchorId`, `AnchorKind` (15 variants including `AnchorRegionName`, `AnchorContinuationName`, `AnchorContinuationUse`, `AnchorFunctionName`, `AnchorBindingDef`, `AnchorBindingUse`, `AnchorKeyword`), `AnchorSpan`, `AnchorSet`, `AnchorIndex`, `PositionIndex`, `SourceLineSpan`.

5. **`lua/moonlift/schema/editor.lua`** (full, 406 lines) — MoonEditor schema. The full LSP surface: `Subject` (SubjectContinuation, SubjectTreeFunc, SubjectBinding, SubjectRegionFrag, etc.), `SymbolFact`, `BindingFact`, `BindingScopeFact`, `ScopedBinding`, `SubjectPick`, `DefinitionResult`, `ReferenceResult`, `DiagnosticFact`, `DiagnosticOrigin`, `HoverInfo`.

6. **`lua/moonlift/schema/lsp.lua`** (full, 160 lines) — MoonLsp schema. Protocol-level types: `ProtocolPosition`, `ProtocolRange`, `Location`, `DiagnosticPayload`, `Hover`, `CompletionList`, `DocumentSymbolPayload`, `SemanticTokens`.

7. **`lua/moonlift/schema/pvm_surface.lua`** (full, 133 lines) — MoonPvmSurface schema for PVM phase definitions. `Producer` variants include `ProducerEmpty`, `ProducerOnce`, `ProducerConcat`, `ProducerCallPhase`, `ProducerChildren`, `ProducerLet`, `ProducerIf`.

### Provenance & Source Tracking

8. **`lua/moonlift/back_provenance.lua`** (full, 105 lines) — **Critical for debugger**. `BackProvenanceMap`: reverse index from BackCmd position to source AST span. Constructed during lowering. Methods: `new()`, `record(cmd_start, cmd_end, source_range, doc_uri, name)`, `resolve(cmd_index)`, `resolve_entry(cmd_index)`, `resolve_range(start_idx, end_idx)`. Stores entries with either a fully-resolved `SourceSpan` or a `name` for later resolution via anchor index.

9. **`lua/moonlift/error/span.lua`** (full, 235 lines) — `SourceSpan` type with construction (`from_source_range`, `from_offsets`, `from_token`, `from_source_text`), queries (`is_point`, `contains`, `overlaps`), and rendering (`render_snippet`, `render_multi_snippet`).

10. **`lua/moonlift/error/span_resolvers.lua`** (full, 575 lines) — Static registry of per-phase span resolver functions. Each phase (parse, host, open, binding, typecheck, backend, link, vec) has a dedicated resolver. The `backend_resolver` uses the provenance map to resolve backend command indices to source spans. It tries: (1) direct span lookup via `provenance:resolve(index)`, (2) name-based resolution via anchor index, (3) named entity fields.

11. **`lua/moonlift/source_position_index.lua`** (140 lines) — Builds position index from document text. Maps between byte offsets and line/column positions. Used everywhere for LSP position resolution.

12. **`lua/moonlift/source_anchor_index.lua`** (111 lines) — Builds sorted index of anchors from `AnchorSet`. Supports lookup by position, range, or id. Anchors sorted by start_offset, then specificity (narrower ranges are more specific).

### Backend Infrastructure

13. **`lua/moonlift/tree_to_back.lua`** (2625+ lines) — **The central lowering pass**. Tree-to-back translation. Creates `BackProvenance.new()` in `with_module_context()` (line 2541). Records provenance at item granularity (line 2604): `lower_context.provenance:record(start_idx, end_idx, nil, nil, name)`. Currently records only item-level names, NOT per-statement or per-block spans. PVM phases: `func_to_back`, `extern_to_back`, `item_to_back`, `module_to_back`.

14. **`lua/moonlift/tree_control_to_back.lua`** (417 lines) — **Critical for debugger**. Lowers `ControlStmtRegion` and `ControlExprRegion` to flat `BackCmd[]`. Creates blocks with `CmdCreateBlock`, `CmdAppendBlockParam`. Generates `CmdJump` between blocks, `CmdBrIf` for if-else, `CmdSwitchInt` for switch. Generates `BackBackBlockId` with structured naming: `"ctl:{nonce}:{region_id}:{label_name}"` — this naming convention preserves semantic information across the lowering. Entry jump becomes `CmdJump(entry_block, args)`. Each block exits with `CmdSealBlock`.

15. **`lua/moonlift/frontend_pipeline.lua`** (250 lines) — Batch compilation pipeline. `parse_and_lower` creates analysis context, builds anchors from scan tokens, calls `lower_module`. Attaches provenance to `analysis_ctx.back_provenance`. Key: anchors track every keyword, name, operator as `AnchorSpan` with precise byte ranges.

16. **`lua/moonlift/back_jit.lua`** (150 lines) — Lua→Rust FFI bridge. `Jit:compile(program)` encodes program to binary wire format, sends to Rust via FFI. Returns `Artifact` with `getpointer(func)`, `getbytes`, `disasm`, `hexbytes`. No debug info or source mapping in the FFI layer.

17. **`lua/moonlift/back_inspect.lua`** (55 lines) — Inspection of `BackProgram`. Counts command kinds, extracts memory/address/alias/pointer offset inspection records. Does NOT include any provenance or source mapping.

18. **`lua/moonlift/back_diagnostics.lua`** (28 lines) — Combines inspection, vector decisions, and disassembly into a `BackDiagnosticsReport`. Calls `Jit:compile(program)` and then `artifact:disasm(func)` for native disassembly.

### LSP/Editor Infrastructure

19. **`lsp.lua`** — LSP server entry point.

20. **`lua/moonlift/lsp_capabilities.lua`** — LSP capabilities JSON: supports hover, documentSymbol, diagnostics (pull), completion, signatureHelp, definition, references, documentHighlight, rename, codeAction, foldingRange, selectionRange, inlayHint, semanticTokens.

21. **`lua/moonlift/rpc_lsp_decode.lua`**, **`lua/moonlift/rpc_lsp_encode.lua`** — JSON-RPC message codec.

22. **`lua/moonlift/rpc_stdio_loop.lua`** — STDIO event loop for LSP.

23. **`lua/moonlift/editor_subject_at.lua`** (120 lines) — Picks the semantic subject at a source position. Resolves anchors to subjects: `SubjectContinuation`, `SubjectTreeFunc`, `SubjectBinding`, `SubjectRegionFrag`, `SubjectKeyword`, etc. This is the semantic "what is at this cursor position" query.

24. **`lua/moonlift/editor_definition.lua`** (63 lines) — Goto-definition via binding facts.

25. **`lua/moonlift/editor_references.lua`** — Find references.

26. **`lua/moonlift/editor_hover.lua`** — Hover information.

27. **`lua/moonlift/editor_symbol_facts.lua`** (145 lines) — Document symbols: functions, types, regions, expr fragments, continuations. Emits `SymbolFact` with ID, parent, name, kind, range. Continuation labels become `SymEvent` kind.

28. **`lua/moonlift/editor_binding_facts.lua`** (260 lines) — Binding resolution: def sites, use sites, scopes.

29. **`lua/moonlift/editor_semantic_tokens.lua`** — Semantic token highlighting.

30. **`lua/moonlift/editor_document_highlight.lua`** — Document highlights.

31. **`lua/moonlift/editor_rename.lua`** — Rename support.

32. **`lua/moonlift/editor_code_actions.lua`** — Code actions.

33. **`lua/moonlift/editor_folding_ranges.lua`** — Folding ranges.

34. **`lua/moonlift/editor_selection_ranges.lua`** — Selection ranges.

35. **`lua/moonlift/editor_inlay_hints.lua`** — Type inlay hints.

36. **`lua/moonlift/editor_completion_context.lua`** — Completion context.

37. **`lua/moonlift/editor_completion_items.lua`** — Completion items.

38. **`lua/moonlift/editor_signature_help.lua`** — Signature help.

### PVM Framework

39. **`lua/moonlift/pvm.lua`** (1200+ lines) — The recording phase virtual machine. Core primitives: `pvm.phase(name, handlers, opts)` creates recording boundaries. Phases auto-cache: on hit, replay cached array; on miss, run recording triplet that lazily evaluates and commits to cache on full drain. `pvm.one()`, `pvm.drain()`, `pvm.drain_into()` are terminals. `args_cache` option controls caching behavior (`"full"`, `"last"`, `"none"`). Phase objects also expose uncached terminals for compiler execution paths.
...
43. **`src/ffi.rs`** (200 lines) — C FFI exports: `moonlift_jit_new`, `moonlift_jit_free`, `moonlift_jit_symbol`, `moonlift_jit_compile_binary`, `moonlift_artifact_getpointer`, `moonlift_artifact_free`. No debugging or inspection APIs.
...
### Control Flow in MoonTree (tree.lua)
...
Source text
    │
    ▼ Parse
MoonTree.Module (AST with StmtJump, StmtYield, ControlStmtRegion, etc.)
    ...
    ▼
Typecheck → MoonTree.Module (typed)
    ▼
Layout → MoonTree.Module (memory layouts resolved)
    ▼
...
back_command_binary.lua: Encode → binary wire format
    ▼
back_jit.lua: FFI → Rust moonlift_jit_compile_binary
    ▼
...
8. **No interactive REPL or step execution mode exists.** `moonlift run --call main file.mlua` compiles and runs — no interactive pause. The pipeline is batch-only.
...
10. **Semantic breakpoints** are natural in the Moonlift paradigm. Instead of "break at line 42", a user could say "break when block `loop` is entered with `i > 5`" or "break on transition from `read_block` to `done`". The `ControlFact` system already knows every edge.
...
3. **Moonlift's explicit control flow is both the problem and the answer** — it creates a debugging paradigm that no traditional debugger can offer, but it also means conventional debugger assumptions (step-over, step-into, line-granularity) don't map naturally.
...
In Moonlift, these concepts don't map cleanly:
...
`ControlFactBackedge` fires when a jump target equals the *entry label* of a region. This is Moonlift's equivalent of "looping back to the start":
...
This has a surprising implication for debugging: **backedge detection is syntactic, not heuristic**. A conventional debugger has to use PC range comparisons or loop detection heuristics to determine if a particular branch is a backedge. Moonlift's debugger knows statically, from the ASDL, whether any particular `jump` is a backedge.
...
|---|---|---|---|
| **MoonTree** (AST) | After typecheck | Statements + blocks | `BlockLabel(name)`, `region_id` |
...
Based on the comprehensive scout and knowledge-builder analysis, here are three genuinely distinct architectural approaches for a Moonlift debugger.
...
- **Single-threaded**: The trap handler suspends the entire JIT. Concurrent Moonlift execution would need per-thread trap state.
...
| 3. Trap instrumentation | New `lua/moonlift/debug_instrument.lua` — walk `BackCmd[]`, insert `CmdTrap` at block entries | ~80 lines |
| 4. Debugger core | New `lua/moonlift/debugger.lua` — breakpoint table, stepping logic, condition eval | ~300 lines |
| 5. DAP server | New `lua/moonlift/dap_server.lua` — DAP protocol parser/handler alongside LSP | ~400 lines |
...
**Core philosophy**: Make Moonlift generate debuggable native code with full DWARF and ValueLabel support, then use existing debuggers (LLDB/GDB) via a Moonlift DAP adapter — leverage the entire LLVM debug ecosystem without reinventing any stepping logic.

**Architecture overview**:
- **Intelligence**: Split — Rust side handles all DWARF generation and native debugging infrastructure; Lua side provides Moonlift-specific semantic queries via the LSP.
...
- **Key insight**: Instead of building a custom debugger, extend Moonlift to produce standard debug info formats. Then LLDB/GDB provide stepping, variable inspection, backtraces, memory inspection — for free. A thin Moonlift DAP wrapper translates between Moonlift concepts (block labels, regions) and the native debugger's view (functions, lines, variables).

**Stepping model** — PC-level with Moonlift semantic overlay:
- The native debugger provides standard step-over/step-into/step-out at the instruction level.
- The DAP adapter adds semantic stepping on top: it queries LSP to determine "what Moonlift construct is at the current PC" and overrides the debugger's step behavior.
- Example: "step over" in Moonlift means "execute until the next block boundary." The adapter sets a temporary breakpoint at all successor blocks' entry PCs and resumes. When one hits, it removes the other temporary breakpoints.
...
**Breakpoint model** — DWARF source-line mapping with Moonlift overlay:
...
- Conditional breakpoints delegate to LLDB's expression evaluation (LLDB can evaluate C expressions against the running process). This requires Moonlift to expose block parameters as named variables in the DWARF info.
...
- Object emission: When emitting `.o` files, include full `.debug_info`, `.debug_line`, `.debug_abbrev` sections. This enables post-mortem debugging with `gdb moonlift_program core` or `lldb -c core.dump`.
...
- They share data through a common "Moonlift context" that maps source ranges ↔ block labels ↔ binding facts. Both the LSP server and the DAP adapter read from the same `analysis_ctx` structure.
...
**What it enables that other approaches can't**:
- **Full native debugging**: Core dumps, memory inspection, register inspection, assembly view, profiling — everything standard debuggers offer, but for Moonlift-compiled code.
...
- **Multi-language debugging**: If Moonlift code calls C libraries or vice versa, LLDB handles mixed-language stack traces, variable inspection, and stepping seamlessly.
- **Post-mortem debugging**: Core dumps from Moonlift programs are fully debuggable because DWARF info is in the binary.

**What it sacrifices or leaves hard**:
- **Semantic stepping requires the DAP adapter to be smart**: LLDB doesn't know about blocks, regions, or backedges. The DAP adapter must constantly translate Moonlift concepts to PC ranges and back. This translation layer is complex and error-prone.
- **No state machine visualization**: LLDB has no concept of "current block in region." The DAP adapter would need to maintain a separate model of the Moonlift state machine and query LLDB for the current PC to map it.
...
- **Variable naming is fragile**: Moonlift values are SSA and get optimized/rematerialized. Getting LLDB to show `i` and `acc` for a block's params at every point in the block requires Cranelift's `ValueLabel` to survive optimization, which it's not guaranteed to do at `opt_level = "speed"`.
...
| 5. LLDB DAP adapter | New `lua/moonlift/dap_lldb_adapter.lua` — manages LLDB process, translates MI protocol ↔ DAP | ~500 lines |
| 6. Semantic overlay | New `lua/moonlift/dap_semantic.lua` — maps block labels to PC ranges, translates "step over block" to temp breakpoints | ~300 lines |
| 7. PVM recompile on breakpoint toggle | `lua/moonlift/debug_recompile.lua` — cached re-lowering with CmdSourceLoc addition | ~100 lines |
...
**LSP integration**:
- The LSP and debugger are separate tools that share the same "Moonlift analysis" object.
...
- **Paradigm shift in debugging**: Instead of "set breakpoint, run, hit breakpoint, inspect, continue," the workflow becomes "run, then explore the complete execution." This matches how Moonlift's explicit control flow makes execution fundamentally more predictable.
...
| 3. Instrumentation for trace | New `lua/moonlift/debug_trace_instrument.lua` — insert `CmdTrap` at EVERY block entry, plus param-recording commands | ~120 lines Lua |
| 4. Trap handler for recording | `src/lib.rs` — on trap, write block ID + param values to ring buffer, resume | ~50 lines Rust |
| 5. Trace file format | New `TRACE_FORMAT.md`, codec `lua/moonlift/trace_codec.lua` | ~100 lines |
| 6. Trace debugger | New `lua/moonlift/debug_trace_explorer.lua` — trace loading, filtering, query engine, stepping navigation | ~400 lines |
| 7. Web/TUI debugger UI | New `lua/moonlift/debug_trace_ui.lua` — terminal UI or web interface (depends on choice) | ~300+ lines |
...
- **Pick Approach A** when you want a functioning debugger this month. It's the smallest, most self-contained change. The block-transition stepping model is the most natural fit for Moonlift's paradigm. The main risk is performance for tight loops.
...
- **Pick Approach C** when you want the fundamentally different debugging paradigm that Moonlift enables — trace-based, queryable, reversible. It's the most innovative and most aligned with Moonlift's philosophy of explicit control flow. The main risk is trace size and the lack of interactive control during execution.
...
| **Philosophy fit** | 5/5 | Excellent. The block-transition stepping model is the natural unit for Moonlift's explicit control flow. Semantic breakpoints on block labels, not source lines. The `ControlFactBackedge` for loop-aware stepping. This embraces the paradigm rather than fighting it. |
...
| **Philosophy fit** | 5/5 | **Best of all three.** This is the approach that most fully embraces Moonlift's explicit control flow paradigm. Block transitions *are* the execution trace. Each trace entry is a complete state snapshot of the state machine. The trace is a first-class artifact that can be shared, analyzed, and replayed. The query language (filter by block, by parameter value, by condition) is the natural way to reason about Moonlift programs. This is the debugging paradigm that Moonlift's architecture makes uniquely possible. |
...
**Summary**: A→C is genuinely incremental. A→B is not — they're architectural alternatives, not phases of the same plan. The user should decide which paradigm fits Moonlift's philosophy (A/C's block-transition model or B's PC/instruction model) before building anything, because the first 200 lines of Rust infrastructure commit you to one path.
...
**Recommended**: **Approach C**, despite its feature incompleteness. It has the cleanest architecture (record/query phase separation), the best philosophical alignment with Moonlift's explicit control flow, and the most natural path to a fundamentally new debugging experience. **But** the trap handler problem must be solved with a different strategy — either using Cranelift's `jump` + explicit condition instead of `trap` (resumable by construction), or by implementing the record phase as a pure Lua interpreter over `BackCmd[]` (no traps at all, just simulation). The signal-handler approach is too fragile.
...
# Moonlift Debugger — Architectural Specification

**Status**: Definitive | **Date**: 2026-05-21 | **Workflow**: wf-moonlift-debugger

---

## Goal

Enable interactive debugging of Moonlift programs at the block-transition level of granularity, using a pure-Lua `BackCmd[]` interpreter for stepping and native Cranelift JIT for fast-path continuation. The debugger provides semantic breakpoints (block-label–based, not source-line–based), parameter-value inspection, block-navigation, and a DAP server that shares the LSP's STDIO loop and analysis context.

---

## Incentives

Moonlift's explicit control flow — where every `block`/`jump`/`yield`/`return` is typed and named — creates a fundamentally different debugging paradigm from conventional PC-level debuggers. The existing infrastructure already captures:
...
Source text → Parse (anchors built with AnchorSpan per token)
  → Typecheck → MoonTree.Module (typed AST with ControlStmtRegion, ControlExprRegion)
    → Layout → MoonTree.Module (memory layouts resolved)
      → tree_to_back.lua: Lower → BackCmd[] (flat command stream)
        → back_validate.lua → back_command_binary.lua: Encode → binary wire format
          → back_jit.lua: FFI → Rust moonlift_jit_compile_binary → Cranelift JIT
            → Executable native code
```

Every stage is synchronous and completes before the next starts. The Rust JIT (`moonlift_jit_compile_binary`) is a single FFI call that compiles the entire wire format at once. There is no concept of "pause," "resume," or "step."

### BackCmd[] — The Flat Command Stream

The target of lowering is `MoonBack.BackProgram` containing `BackCmd[]` — a flat array of commands. Key control-flow commands (from `lua/moonlift/schema/back.lua`):
...
│  (lua/moonlift/dap_server)   │     │  (lsp.lua + existing modules)│
│                              │     │                              │
...
### 2. BackCmd Interpreter (`lua/moonlift/debug_interpreter.lua`)
...
`CmdCall` with `BackCallExtern` target: the interpreter looks up the extern function by name in the `extrn` table (populated from `CmdDeclareExtern` declarations or from the compiler's extern registry). It calls the FFI function with the operand values, captures the return value, and stores it in the result register. The extern registry is populated by the host pipeline (`moonlift.host` → extern function table) — the interpreter receives this as a constructor parameter.

`CmdCall` with `BackCallDirect`: calls another Moonlift function within the same module. The interpreter maintains a `functions` table mapping `BackFuncId` → `BackCmd[]` (the function's body). On call: push the current cursor and register file onto a call stack, set cursor to the callee's body start, and resume interpretation. On return: pop the call stack, restore cursor, capture return value.
...
- **Uninitialized registers**: If a jump target receives arguments with names that weren't set by the caller (shouldn't happen in valid Moonlift), the interpreter provides a typed default (0 for integers, false for booleans, 0.0 for floats).
...
### 3. Debugger Core (`lua/moonlift/debugger_core.lua`)
...
### 4. DAP Server (`lua/moonlift/dap_server.lua`)
...
### 5. LSP Integration (`lua/moonlift/dap_breakpoint_resolver.lua`)

**Responsibility**: Bridge the DAP's source-line–based breakpoint model to Moonlift's block-label–based breakpoint model. Map `{uri, line}` → `{region_id, block_label}`.
...
### 6. Native Fast Path (`lua/moonlift/debug_native_exec.lua` — Future)
...
In Moonlift, there are no function calls within control flow blocks (all calls are either extern FFI calls or direct function calls, both of which are visible as `CmdCall` commands). "Step Into" behaves identically to "Step Over" for control flow — it executes the current block and stops at the next block boundary.

If the current statement is a `CmdCall(dest, target, args)` to a Moonlift function, "Step Into" follows into the callee's first block. The interpreter pushes the current cursor onto the call stack and sets cursor to the callee's body.
...
```lua
-- In moonlift/host.lua (or equivalent extern provider):
...
The `extrn` table is populated from the same extern declarations that the Rust JIT uses. For the hosted pipeline (`moonlift.loadstring`), these are LuaJIT FFI function handles. For the MOM pipeline, they're native function pointers. The interpreter doesn't care about the implementation — it just calls them.
...
The recompilation is fast because PVM phases cache their outputs. Only the `moonlift_tree_module_to_back` phase and downstream phases need to re-run for the changed file.
...
### How does the interpreter handle CmdCall to direct Moonlift functions?

When `CmdCall(dest, target, args)` targets a direct Moonlift function (not an extern), the interpreter:
...
The call stack is also what the DAP's `stackTrace` returns — it shows both the Moonlift call frames and the block breadcrumbs within each frame.
...
2. **Interpreter skeleton** (`lua/moonlift/debug_interpreter.lua`, ~200 lines): Implement the core interpreter loop, register file, block boundary detection, and basic command handlers (control flow, constants, arithmetic, comparisons).

3. **Debugger core skeleton** (`lua/moonlift/debugger_core.lua`, ~100 lines): State machine, stepping modes, `pause()`/`continue()`/`step_block()`.

### Phase 2: Breakpoints and LSP Bridge (~200 lines)

4. **Breakpoint resolver** (`lua/moonlift/dap_breakpoint_resolver.lua`, ~80 lines): Map source lines → `AnchorContinuationName` → `BlockLabel` using anchor index.
...
9. **Moonlift function calls** in interpreter (`debug_interpreter.lua`, ~50 lines): Call stack management, direct function body lookup.

### Phase 4: DAP Server (~400 lines)

10. **DAP server** (`lua/moonlift/dap_server.lua`, ~300 lines): DAP protocol implementation with all required requests and events.

11. **STDIO loop integration** (`rpc_stdio_loop.lua`, ~30 lines): Add DAP method dispatching.

12. **Variable inspection** (`lua/moonlift/dap_variables.lua`, ~70 lines): Format block parameter values with types from binding facts.
...
14. **Trace explorer** (`lua/moonlift/debug_trace_explorer.lua`, ~250 lines): Timeline view, filter by block/param value/condition, reverse-step navigation.
...
## Implementation Plan: Moonlift Debugger (Semantic Block-Trace)

### Precondition Checks

Before any edits begin, verify:
1. `lua/moonlift/tree_control_to_back.lua` still has `lower_region` at line 353 with 6 params (no provenance param added yet)
2. `lua/moonlift/tree_to_back.lua` still has `control_api` defined at line 2159 with the current `base` table
3. `lua/moonlift/tree_to_back.lua` still has `lower_module_direct` at line 2596 and item-level provenance at line 2604
4. `lua/moonlift/rpc_stdio_loop.lua` still has `run()` at line 30 with the current message-dispatch loop
5. `lua/moonlift/frontend_pipeline.lua` still creates `analysis_ctx.anchors` and passes it through to lowering
6. Check that `lua/moonlift/back_provenance.lua` `record()` function signature is `record(cmd_start, cmd_end, source_range, doc_uri, name)` — if name is provided but source_range is nil, it stores a name-only entry

---

### Phase 1: Foundation

#### 1.1 `lua/moonlift/tree_to_back.lua` — Thread provenance into the `base` table passed to `tree_control_to_back`
...
#### 1.2 `lua/moonlift/tree_control_to_back.lua` — Record block-level provenance
...
#### 1.3 NEW: `lua/moonlift/debug_interpreter.lua` — Interpreter skeleton
...
function Interpreter:_dispatch(cmd, cls)
    local Back = _G.Back or require("moonlift.schema.back")  -- cached at init
...
#### 1.4 NEW: `lua/moonlift/debugger_core.lua` — Debugger core
...
#### 2.1 NEW: `lua/moonlift/dap_breakpoint_resolver.lua` — Source line → block label mapping

**Purpose**: Map DAP source-line breakpoints to Moonlift block labels using the anchor index. Bridges the PC-level debugger model to the block-level debugger model.
...
**Dependencies**:
- `moonlift.source_position_index`
- `moonlift.source_anchor_index`
...
```lua
    ...
    if self.opts.anchor_set and self.opts.source_uri and self.opts.source_text then
        ...
        local S = _G.T and _G.T.MoonSource  -- schema context from caller
...
#### 3.1 `lua/moonlift/debug_interpreter.lua` — Add memory simulation handlers
...
#### 3.2 `lua/moonlift/debug_interpreter.lua` — Add remaining command handlers
...
#### 4.1 NEW: `lua/moonlift/dap_server.lua` — DAP server
...
-- Lazy-required: Resolver = require("moonlift.dap_breakpoint_resolver")
...
--   functions: {[func_id] = BackCmd[]} — Moonlift function bodies
...
function DapServer:_write_json(obj)
    ...
    local json = require("moonlift.rpc_json_encode")
...
function DapServer:_handle_set_breakpoints(request_seq, args)
    ...
    local resolved = {}
    local resolver_module = require("moonlift.dap_breakpoint_resolver")
...
**Dependencies**:
- `moonlift.debugger_core`
- `moonlift.dap_breakpoint_resolver` (lazy)
- `moonlift.rpc_json_decode` / `moonlift.rpc_json_encode`
- `pvm`
...
**Edge cases**:
- DAP uses 1-based line numbers; Moonlift anchors use 0-based (LSP convention). Convert with `line - 1`.
...
#### 4.2 `lua/moonlift/rpc_stdio_loop.lua` — Add DAP method dispatching
...
#### 4.3 NEW: `lua/moonlift/dap_variables.lua` — Variable formatting
...
```lua
-- moonlift/dap_variables.lua
-- Format Moonlift variables for DAP display.
...
8. **No globals**: Import schema classes via local variable at module init, e.g., `local Back = T.MoonBack` inside `Define(T)` functions. For the interpreter (which doesn't use `Define(T)`), capture the `Back` table at construction time: `self.Back = Back`.
...
Implemented the Moonlift Debugger — a pure-Lua block-transition debugger with interpreter, breakpoints, stepping, and DAP server. The debugger operates at the block-transition level, using Moonlift's explicit control flow to provide semantic breakpoints (block-label based), parameter value inspection, and block navigation — all without any Rust code changes beyond the pre-existing provenance infrastructure.

## Files Changed

### Modified (3 files):
- **`lua/moonlift/tree_to_back.lua`** — Added `get_provenance` accessor to the `base` table passed to `tree_control_to_back.Define(T, base)`, enabling block-level provenance recording in the control flow lowering pass.
- **`lua/moonlift/tree_control_to_back.lua`** — Added provenance recording: (a) after each `CmdCreateBlock` in `declare_blocks()`, record `"block:{label_name}"`; (b) after each `CmdSwitchToBlock` in `lower_region()`, record `"switch:{label_name}"`. This builds name-based provenance entries the debugger uses for block-to-source mapping.
- **`lua/moonlift/rpc_stdio_loop.lua`** — Added DAP method dispatching: checks if incoming messages are DAP methods (via `dap_server.is_dap_method`) and routes them to the DAP handler, while all other messages continue to the existing LSP dispatch. Optional `dap_handler` passed through `opts`.
...
### New files (6 files):
- **`lua/moonlift/debug_interpreter.lua`** (~1050 lines) — The heart of the debugger. Pure-Lua `BackCmd[]` interpreter with:
  ...
  - Calls: extern FFI calls via `pcall`, direct Moonlift function calls with call stack
  - Stepping protocol: `step_block()`, `continue_until(breakpoints_fn)`, `pause()`
  - Block boundary maps, function body maps built during init

- **`lua/moonlift/debugger_core.lua`** (~160 lines) — Debugger state machine:
...
- **`lua/moonlift/dap_breakpoint_resolver.lua`** (~100 lines) — Maps DAP source-line breakpoints to Moonlift block labels using the anchor index. Uses `PositionIndex` and `AnchorIndex` from the existing LSP infrastructure. Provides `resolve_line(doc_uri, line, source_text, anchor_set)` and `resolve_block_label(block_label, anchor_set)`.

- **`lua/moonlift/dap_server.lua`** (~450 lines) — Full DAP protocol server:
...
- **`lua/moonlift/dap_variables.lua`** (~90 lines) — Variable formatting for DAP display. Infers types from Lua value types (bool, i32, i64, f64, ptr) and formats values for display.

- **`lua/moonlift/debug_init.lua`** (~45 lines) — Helper module for creating a Debugger instance from a compiled `BackProgram` and analysis context. Bridges the compilation pipeline to the debugger.
...
2. **Block-level stepping**: The natural fit for Moonlift's jump-first semantics

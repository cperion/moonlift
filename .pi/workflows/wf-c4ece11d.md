...
5. `experiments/lua_interpreter_vm/spongejit/lua_compile/lua_exec_to_lalin_cfg_lower.lua` (lines 412-438, 461-475, 548-567)
   - LuaExec object expressions lower into LalinCFG runtime ops:
     `RuntimeTableRawGet`, `RuntimeTableRawSetCanWrite`, `RuntimeLenNoMeta`, `RuntimeStringConcat2`, `RuntimeTableRawSet`.

6. `experiments/lua_interpreter_vm/spongejit/lua_compile/lalin_cfg_emit.lua` (lines 312-380, 509-525, 602-630)
   - Emits raw hash access directly into Lalin source.
...
7. `lua/lalin/tree_to_back.lua` (lines 814-824, 1048-1072, 1377-1409)
...
9. `lua/lalin/mlua_run.lua` (lines 16-62, 77-128)
   - Native call path:
     `lalin.loadstring` → parse/lower → JIT compile → `ffi.cast(csig, ptr)` → `CompiledFunction:__call`.

10. `lua/lalin/back_jit.lua` (lines 100-184)
...
11. `experiments/lua_interpreter_vm/spongejit/ssa_asdl/spongejit_lua_ssa.asdl` (lines 496-795, 1180-1536)
    - ASDL definitions for LuaRT values/table states, LuaExec table expressions/ops, and LalinCFG runtime table/string ops.

12. `experiments/lua_interpreter_vm/spongejit/lua_compile/lalin_cfg_validate.lua` (lines 160-224, 242-274)
...
16. Generated `/tmp/test_rt_gettable_raw_hit.mlua` (lines 1-23)
    - Emitted Lalin source for first failing native function.
...
→ lua_exec_to_lalin_cfg_lower
    CFG.RuntimeTableRawGet
    CFG.RuntimeRawGetValueOrNil
→ lalin_cfg_emit
    Lalin source with LuaRTTable/hash declarations and nested select/hash probes
→ lalin.loadstring
→ mlua_run.compile
...
```text
lalin_cfg_emit emits select(cond, unsafe_hash_load, fallback)
...
- The LalinCFG validator validates semantic/runtime value shape but not host FFI fixture layout compatibility or non-null pointer preconditions.
...
3. Update `experiments/lua_interpreter_vm/spongejit/lua_compile/lalin_cfg_emit.lua`:
...
- `experiments/lua_interpreter_vm/spongejit/lua_compile/lalin_cfg_emit.lua` - replace unsafe eager hash access emission with guarded block expressions/statements.
...
## Risks
- Lalin boolean `and/or` and `select(...)` are eager at lowering time; do not use them as memory guards.
...
- Reworked table raw get/set emission to avoid eager `select` / boolean guarding around unsafe hash loads. Hash access is now behind explicit Lalin control with null/capacity checks.
...
- `experiments/lua_interpreter_vm/spongejit/lua_compile/lalin_cfg_emit.lua` - guarded raw get, raw set can-write, and raw set hash access paths.

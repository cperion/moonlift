# Lalin WASM Compilation Target
Explore what it would take to add WASM as a compilation target for Lalin, using the existing Cranelift backend infrastructure.
...
10. `Makefile` (lines 1-67) — builds static `lalin`, `mom`, shared `liblalin`.
11. `BACK_WIRE_FORMAT.md` (lines 1-454) — Flatline v4 spec, tag table, declarations.
12. `lua/lalin/back_command_binary.lua` (lines 1-751) — Lua encoder from `BackProgram` to binary wire format.
13. `lua/lalin/schema/back.lua` (lines 1-1000, 500-800) — `LalinBack` schema: scalars, targets, BackCmd variants.
14. `lua/lalin/back_validate.lua` (lines 1-968) — BackCmd validation, target supported shape checks, memory constraints.
15. `lua/lalin/tree_to_back.lua` (lines 1-2616) — source tree lowering to BackCmd; functions, externs, memory, views, ABI.
16. `lua/lalin/tree_control_to_back.lua` (lines 1-415) — jump-first regions/blocks/yield lowered to BackCmd CFG.
17. `lua/lalin/type_to_back_scalar.lua` (lines 1-86) — source type → `BackScalar`.
18. `lua/lalin/type_size_align.lua` (lines 1-141) — scalar/pointer/view layout; currently 64-bit pointer/index.
19. `lua/lalin/type_func_abi_plan.lua` (lines 1-83) — executable ABI; views lower to `(ptr,index,index)`.
20. `lua/lalin/back_target_model.lua` (lines 1-157) — default native target model; 64-bit, Cranelift JIT, SIMD facts.
21. `lua/lalin/back_jit.lua` (lines 1-175) — Lua FFI wrapper for Rust JIT; encodes binary wire.
22. `lua/lalin/back_object.lua` (lines 1-93) — Lua FFI wrapper for Rust object emission.
23. `lua/lalin/frontend_pipeline.lua` (lines 1-260) — `.mlua` parse/typecheck/lower/validate pipeline.
24. `lua/lalin/hosted_jit.lua` (lines 1-60) — hosted binary path using `_host_compile_binary`.
25. `lua/lalin/schema/link.lua` (lines 1-300) — link schema already has `LinkPlatformWasm`, `LinkArchWasm32`, `LinkFormatWasm`.
26. `lua/lalin/link_target_model.lua` (lines 1-52) — host-only link target model.
27. `lua/lalin/link_command_plan.lua` (lines 1-116) — `cc`/`ar` command generation for native objects/shared libs.
28. `lua/lalin/link_execute.lua` (lines 1-60) — executes linker commands.
29. `lua/lalin/init.lua` (lines 1-170) — public `emit_object` / `emit_shared`.
...
let isa_builder = cranelift_native::builder()
    .map_err(|e| LalinError(format!("host machine is not supported by Cranelift: {e}")))?;
```
...
- `tree_to_back.lua` emits a flat `LalinBack.BackProgram`.
- `back_command_binary.lua` encodes `BackProgram` into Flatline binary bytes.
- Lua JIT path:
  `back_jit.lua` → `lalin_jit_compile_binary` FFI → `Jit::compile_binary` → `decode_module` into `JITModule`.
- Object path:
  `back_object.lua` → `lalin_object_compile_binary` FFI → `compile_object_binary` → `decode_module` into `ObjectModule`.
- Link path:
  `emit_shared.lua` / `lalin.emit_shared` emits host object then builds `LinkPlan` using `cc` for shared library.
...
- Natural WASM scalar mapping:
  ...
  - `ptr/index` for wasm32 should likely be `i32`; current Lalin code hardcodes 64-bit pointer/index in several Lua layout/target files.
...
- Host arena is native-pointer based and not currently exposed in `src/ffi.rs` despite Lua cdefs for `lalin_host_*`; mapping it to WASM would need a linear-memory/object-handle story.
...
- **Memory/address model**: Lalin currently treats pointers as native raw addresses; WASM treats them as linear-memory offsets, while functions/tables/memory are separate namespaces.
- **Control-flow restructuring**: Lalin’s flat jump-first CFG maps naturally to Cranelift blocks, but not directly to structured WASM control.
...
- **Philosophy fit**: Lalin’s explicit ASDL/flat backend story is an opportunity, but current host-native defaults and no-op semantics conflict with “fail fast, fail loud”.
...
Lalin currently maps callable types to `BackPtr`, and `CmdFuncAddr` / `CmdExternAddr` produce `BackPtr` values. That creates a deep mismatch: if `BackPtr` becomes a linear-memory address in WASM, function pointers cannot remain the same scalar without an additional interpretation layer.
...
The source-level extern syntax supports `as "symbol"`, but that is still a single string. Encoding `"env.write"` into one symbol would hide ASDL meaning in strings, which conflicts with Lalin’s explicit-ASDL design principle.
...
Lalin’s jump-first model is an advantage: all blocks, jumps, and branch args are explicit before backend emission.
...
This creates a philosophical tension: Lalin’s backend boundary is flat and grep-shaped, while WASM emission may require an internal structured reconstruction pass.
...
#### 16. Lalin’s design philosophy favors a WASM backend, but exposes current inconsistencies

Philosophically, Lalin has a strong advantage: the backend consumes explicit, flat, verified commands rather than hidden AST callbacks or stringly side tables.
...
- Whether arbitrary Lalin CFG must be supported, or whether lowering can guarantee reducible/structured CFG for source-produced programs.
...
- **Key changes**:
  ...
  - Add `lalin.emit_wasm(...)` Lua API alongside `emit_object`.
...
- **Tradeoff**: Optimizes for preserving Lalin’s flat-command architecture and reusing the existing frontend/lowering path; sacrifices Cranelift reuse for WASM emission because Cranelift cannot emit WASM.
...
- **Key changes**:
  - Add `lua/lalin/schema/wasm.lua` with explicit module/function/import/memory/table/control nodes.
  - Add `tree_to_wasm.lua` or `back_to_wasm.lua` that lowers typed Lalin directly to WASM-shaped IR.
...
- **Tradeoff**: Optimizes for semantic correctness and Lalin’s “explicit ASDL meaning” philosophy; sacrifices reuse of the current Rust decoder and creates a larger second backend.
...
- **Rough sketch**:
  ...
  - Lower Lalin regions/control into a structured WASM ASDL, not arbitrary Cranelift-style blocks.
...
- **Core idea**: Add a WASM path that lowers Lalin to C-like or LLVM-like IR and delegates WASM code generation, CFG structuring, linking, and object emission to LLVM/clang/wasm-ld.

- **Key changes**:
  - Add an alternate backend emitter: `BackCmd[] -> LLVM IR` or `Lalin typed IR -> C/LLVM`.
...
- **Risk**:
  - Harder to keep Lalin’s backend semantics grep-shaped and fully explicit.
...
- Pick **Approach A** if the priority is preserving Lalin’s current flat backend boundary while building a real native `.wasm` emitter.
...
- Pick **Approach C** if the priority is getting a usable WASM artifact soon, accepting LLVM/clang as a pragmatic external backend rather than making WASM a fully native Lalin backend.
...
| **Philosophy fit** | 4/5 | Preserves Lalin’s flat backend-command architecture and can make target facts explicit. Less ideal because WASM structuring/verification would happen behind the flat boundary. |
...
| **Philosophy fit** | 5/5 | Best match for Lalin: explicit ASDL meaning, no hidden string metadata, no external semantic escape hatch, and target semantics represented as typed compiler data. |
...
| Dimension | Score (1-5) | Rationale |
|-----------|-------------|-----------|
| **Coupling** | 4/5 | Couples Lalin semantics to LLVM IR, clang/wasm-ld behavior, flags, triples, and toolchain versions. The backend boundary becomes partly external. |
| **Cohesion** | 2/5 | Responsibilities are split awkwardly: Lalin emits partial semantics, LLVM handles CFG/legalization/codegen, wasm-ld handles artifact semantics. Harder to reason about as one compiler. |
...
| **Philosophy fit** | 2/5 | Weak fit. It hides important semantics in LLVM/toolchain behavior and conflicts with Lalin’s explicit-ASDL, fail-loud, grep-shaped design. |
...
| **Maintainability** | 2/5 | Likely fastest to demo, but hardest to keep principled. Debugging semantic mismatches across Lalin → LLVM → wasm-ld would be expensive. |
...
**Key concern**: Avoid outsourcing Lalin’s semantic contract to LLVM in a way that makes backend behavior non-explicit and version-dependent.

---

### Summary

- **Recommended**: **Approach B — WASM-Native ASDL Backend Before Flatline**. It has the highest migration cost, but best matches Lalin’s architecture and philosophy. WASM is semantically different enough from native Cranelift that it deserves a first-class target IR.
...
- **Avoid**: **Approach C — External Toolchain / LLVM-WASM Bridge**. It is the most pragmatic short-term path, but weakest architecturally and least aligned with Lalin’s explicit compiler-data model.
...
This matters because Lalin’s design philosophy relies on explicit ASDL meaning and fail-fast verification boundaries. A WASM target that hides semantics in symbol strings, target-specific side channels, or an external toolchain would conflict with that model. The chosen design makes WASM a real semantic target instead of overloading the existing Cranelift-native backend.

## Current State

Lalin’s compilation pipeline is:
...
- `lua/lalin/tree_to_back.lua`
  - Lowers typed Lalin source into `LalinBack.BackProgram`.
...
- `lua/lalin/schema/back.lua`
  - Defines the `LalinBack` ASDL schema: scalars, targets, BackCmd variants, memory metadata, signatures, externs, functions, blocks, values, and data declarations.
...
pub fn compile_object_binary(
    payload: &[u8],
    module_name: &str,
) -> Result<ObjectArtifact, LalinError> {
    let isa = host_isa(true)?;
    let builder = ObjectBuilder::new(isa, module_name, default_libcall_names())?;
    let mut module = ObjectModule::new(builder);
...
This approach was selected because it has the best architectural fit with Lalin’s core principles:
...
```text
lua/lalin/tree_to_wasm.lua
```

or, where appropriate after typed backend lowering,

```text
lua/lalin/back_to_wasm.lua
```
...
- conversion from Lalin jump-first control into structured WASM control;
...
This avoids hiding WASM import semantics inside strings such as `"env.write"` and preserves Lalin’s explicit-ASDL rule.
...
Lalin source uses jump-first control. The native backend lowers this into arbitrary Cranelift CFG blocks:
...
The chosen design gives the WASM backend its own structured control representation in the WASM ASDL. The control lowering/validation boundary is part of the WASM path, not hidden inside the Flatline decoder. Source-produced Lalin control must be represented in the WASM schema in a form suitable for WASM emission, with unsupported or invalid shapes rejected by the WASM validation boundary.
...
```text
Native backend: typed Lalin → BackCmd/Flatline → Cranelift
WASM backend:   typed Lalin → LalinWasm ASDL → WASM encoder
```
...
- **Control complexity**: Lalin’s explicit CFG must be lowered or constrained into WASM-compatible structured control.
...
Approach C would lower Lalin to C-like or LLVM-like IR and use `clang --target=wasm32-wasi`, LLVM’s WASM backend, and/or `wasm-ld` to produce WASM artifacts.
...
It likely offers the fastest route to a demonstrable WASM artifact and avoids writing a WASM encoder and CFG structurer. However, it couples Lalin semantics to LLVM IR, clang/wasm-ld behavior, flags, triples, and toolchain versions. It also weakens Lalin’s explicit-ASDL architecture by outsourcing important semantic decisions to an external backend.

The critique rated it weakest in philosophy fit, cohesion, and maintainability. It was rejected because it hides too much of Lalin’s semantic contract in toolchain behavior and makes backend correctness version-dependent.

## Decision Summary

Lalin will pursue WASM support through a **WASM-native ASDL backend before Flatline**.

Flatline remains the native Cranelift backend boundary. WASM gets its own explicit schema, lowering path, validation boundary, target-parametric ABI/layout handling, and binary emission path. This choice has the highest upfront cost, but best preserves Lalin’s architecture: meaningful compiler state remains typed, explicit, inspectable, and target-correct before artifact emission.

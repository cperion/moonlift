# Moonlift Header Files — Architecture Exploration 
...
1. **`lua/moonlift/schema/tree.lua`** (full file, 1073 lines) — Core ASDL schema for all MoonTree types: Expr, Stmt, Func, ExternFunc, Item, Module, etc.
2. **`lua/moonlift/schema/open.lua`** (full file) — MoonOpen schema: slots, fills, open sets, meta-facts, validation
3. **`lua/moonlift/schema/host.lua`** (full file) — MoonHost schema: host decls, struct decls, exposure, access plans, layouts
4. **`lua/moonlift/schema/type.lua`** (lines 1-60) — TypeRef variants including TypeRefGlobal
5. **`lua/moonlift/schema/core.lua`** (lines 1-15) — Path and Name product types
6. **`lua/moonlift/parse.lua`** — Extern parsing (lines 1542-1555), module parsing (lines 2416-2455), island dispatch (lines 2291-2296)
7. **`lua/moonlift/frontend_pipeline.lua`** (full file) — Batch compilation pipeline: parse → open-expand → open-validate → closure-convert → typecheck → layout-resolve → tree-to-back → back-validate
8. **`lua/moonlift/mlua_document_analysis.lua`** (full file) — LSP/document analysis pipeline for `.mlua` files
9. **`lua/moonlift/tree_module_type.lua`** (full file) — Module environment construction: name resolution, value/type entries, env building
10. **`lua/moonlift/tree_typecheck.lua`** — Typecheck dispatch for Item variants (lines 859-869)
11. **`lua/moonlift/tree_to_back.lua`** — Lowering dispatch (lines 2443-2468, 2477-2523) including extern lowering and global context collection
12. **`lua/moonlift/host_session.lua`** (first 120 lines) — Session/builder API foundation
13. **`lua/moonlift/host_module_values.lua`** (full file) — Bundle/Module builder: item collection, compilation, moduletree assembly
14. **`lua/moonlift/host_func_values.lua`** (lines 1-350) — Func value creation including `_module_extern_func` (line 318)
15. **`lua/moonlift/mlua_run.lua`** (full file) — `.mlua` runtime: compiles source through frontend_pipeline → JIT → callable function
16. **`lua/moonlift/init.lua`** (lines 1-80) — Public API facade: loadstring/loadfile/dofile/eval
17. **`lua/moonlift/ast.lua`** (line 993) — ImportItem wrapping in ast.lua
18. **`lua/moonlift/editor_symbol_facts.lua`** (lines 122-124) — ItemImport symbol extraction for LSP
...
```lua
A.sum "Item" {
    A.variant "ItemFunc"     { A.field "func" "MoonTree.Func" },
    A.variant "ItemExtern"   { A.field "func" "MoonTree.ExternFunc" },
    A.variant "ItemConst"    { A.field "c" "MoonTree.ConstItem" },
    A.variant "ItemStatic"   { A.field "s" "MoonTree.StaticItem" },
    A.variant "ItemImport"   { A.field "imp" "MoonTree.ImportItem" },
    A.variant "ItemType"     { A.field "t" "MoonTree.TypeDecl" },
    A.variant "ItemUseTypeDeclSlot" { A.field "slot" "MoonOpen.TypeDeclSlot" },
    A.variant "ItemUseItemsSlot"    { A.field "slot" "MoonOpen.ItemsSlot" },
    ...
    A.variant "ItemData" { A.field "data" "MoonTree.DataItem" },
}
```
...
A.product "Path" { A.field "parts" (A.many "MoonCore.Name"), A.unique }
```
...
3. **`TypeRefGlobal` is alive in the builder API** but has **no source syntax** — you can't write `let x: "mod".Foo` or `import mod.Foo` in Moonlift source. Instead, users write `@{mod.Foo}` splices.

4. **The extern function pattern** (`extern func name() -> T end`) is the closest existing model for a "header": bodyless, type-signature-only, declares an external dependency. But it has no module path — just a linker symbol name. A Moonlift header would need to carry a module path and a type-checked signature that another `.mlua` file implements.

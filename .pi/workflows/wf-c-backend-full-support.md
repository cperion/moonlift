...
- `lua/moonlift/c_coverage.lua` - new status model and reclassification.
- `lua/moonlift/c_validate.lua` - validates coverage statuses from `Coverage.statuses()`.
- `lua/moonlift/frontend_pipeline.lua` - C phase-boundary guard; layout env passed into C lowering.
- `lua/moonlift/tree_typecheck.lua` - surface casts to machine casts, tagged-union type/layout typing, ArrayLenExpr rejection.
- `lua/moonlift/tree_module_type.lua` - enum/tagged-union `__tag`/`__payload` layout facts.
- `lua/moonlift/sem_layout_resolve.lua` - resolves `ExprCtor` args.
- `lua/moonlift/tree_to_back.lua` - partial native lowering for `ExprCtor` and statement variant switches.
- `lua/moonlift/type_to_c.lua` - ArrayLenExpr diagnostic clarified.
- `lua/moonlift/c_helpers.lua` - fixed signed AShr and unsigned div/rem helper behavior.
...
## Files Changed
- `lua/moonlift/schema/tree.lua` - added `ExprSwitch.variant_arms`
- `lua/moonlift/parse.lua` - parses `case .variant(bind)` switch arms
- `lua/moonlift/tree_typecheck.lua` - preserves/types expression variant arms
- `lua/moonlift/tree_to_back.lua` - native variant switch expression lowering
- `lua/moonlift/tree_control_facts.lua` - emits variant control facts
- `lua/moonlift/tree_control_to_back.lua` - native control variant switch lowering
- `lua/moonlift/tree_to_c.lua` - C ExprCtor, variant switches, view stride indexing, aggregate let init
- `lua/moonlift/tree_control_to_c.lua` - C control variant switch lowering
- `lua/moonlift/c_layout.lua` - layout-backed enum/tagged-union declarations
- `lua/moonlift/c_emit.lua` - type forwards before signatures
- `lua/moonlift/c_coverage.lua` - tagged-union rows marked supported
...
## Files Changed
- `lua/moonlift/c_tcc.lua` - optional libtcc LuaJIT FFI binding.
...
- `MOONLIFT_C_BACKEND_FINAL=1 luajit tests/test_c_backend_coverage_matrix.lua`
- `luajit tests/test_c_gcc_feature_corpus.lua`
- `MOONLIFT_C_CC=cc luajit tests/test_c_gcc_feature_corpus.lua`
- `luajit tests/test_c_gcc_harness.lua`
- `MOONLIFT_C_USE_LIBTCC=1 luajit tests/test_c_gcc_harness.lua`

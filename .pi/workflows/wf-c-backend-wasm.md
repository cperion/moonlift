...
Explore how to design a robust C backend that consumes Moonlift's validated backend commands / wire format and can target native C and eventually WASM while keeping Cranelift.
...
2. `lua/moonlift/schema/back.lua` (lines 1-1398) — Source ASDL schema for `MoonBack`: scalars, shapes, target facts, memory/provenance facts, commands, validation issues, inspection products, `BackProgram`.
3. `lua/moonlift/back_command_binary.lua` (lines 1-759) — Lua encoder from validated `BackProgram` to Flatline binary consumed by Rust.
...
7. `src/ffi.rs` (lines 1-216) — C FFI surface used by LuaJIT (`moonlift_jit_compile_binary`, `moonlift_object_compile_binary`, artifact APIs).
8. `lua/moonlift/back_jit.lua` (lines 1-189) — LuaJIT bridge to Rust JIT binary-wire compile path.
9. `lua/moonlift/back_object.lua` (lines 1-93) — Lua bridge to Rust object emission via binary wire.
10. `lua/moonlift/back_validate.lua` (lines 1-968) — Validation contract for `BackProgram`: refs, duplicates, command ordering, shape/memory constraints.
11. `lua/moonlift/tree_to_back.lua` (lines 1-2729) — Main frontend lowering to backend commands: expressions/statements/control/function ABI/data/external declarations.
12. `lua/moonlift/tree_control_to_back.lua` (not read) — Referenced by `tree_to_back`; likely relevant for region/control-specific lowering if continuing.
13. `lua/moonlift/type_to_back_scalar.lua` (lines 1-86) — Source type → backend scalar mapping.
14. `lua/moonlift/type_size_align.lua` (lines 1-141) — Layout sizes/alignment for scalars, pointers, views, closures, arrays/aggregates.
15. `lua/moonlift/type_abi_classify.lua` (lines 1-72) — ABI class decisions: direct scalar/pointer, descriptors, indirect aggregate.
16. `lua/moonlift/type_func_abi_plan.lua` (lines 1-83) — Function ABI plan: view flattening, aggregate-by-pointer, return handling.
17. `lua/moonlift/back_target_model.lua` (lines 1-157) — Default native target facts and vector target conversion.
18. `lua/moonlift/frontend_pipeline.lua` (lines 1-203) — Parse/typecheck/layout/lower/validate pipeline; rejects `CmdTrap` unless allowed.
19. `lua/moonlift/init.lua` (lines 1-181) — Public API for hosted load, object/shared emission.
20. `lua/moonlift/hosted_jit.lua` (lines 1-75) — In-process hosted Rust binary compile path for standalone binary.
21. `src/main.rs` (lines 1-224) — Standalone `moonlift` binary embeds Lua and exposes `_host_compile_binary`.
...
24. `lua/moonlift/link_target_model.lua` (lines 1-42), `link_*` files — Object/shared library linking model and system `cc` command planning.
25. `lua/moonlift/back_inspect.lua` (lines 1-57), `back_diagnostics.lua` (lines 1-37) — Backend inspection/diagnostics and optional disassembly.
26. `lua/moonlift/host_layout_resolve.lua` (lines 1-254) — Host layout/C ABI facts for exposed structs/views.
27. `lua/moonlift/host_c_emit_plan.lua` (lines 1-31) — Existing C header emission for host layouts only, not a codegen backend.
28. `lua/moonlift/host_view_abi_plan.lua` (lines 1-114) — Host view descriptor ABI planning.
...
`lua/moonlift/schema/back.lua` defines the semantic command set. Critical command families:
...
`lua/moonlift/back_validate.lua` enforces:
...
`lua/moonlift/back_command_binary.lua` maps ASDL commands to flat u32 tags/slots.
...
lib.moonlift_jit_compile_binary(self._raw, buf, #payload)

-- back_object.lua
local payload = binary_api.encode(program)
lib.moonlift_object_compile_binary(buf, #payload, module_name, out)
```
...
Moonlift source
  ...
  -> Rust FFI moonlift_jit_compile_binary / moonlift_object_compile_binary
...
package.preload["moonlift.back_jit"] = hosted_jit
hosted_jit encodes binary and calls _host_compile_binary
_host_compile_binary calls moonlift::Jit::compile_binary
```
...
- **Object FFI free signature mismatch.**
  - Lua declares `moonlift_bytes_free(uint8_t* data, size_t len)`.
  - Rust defines `moonlift_bytes_free(bytes: *mut moonlift_bytes_t)`.
...
- **CFG/block-param preservation**, since Moonlift backend commands are jump-first SSA, not structured AST.
...
- **The wire currently encodes Cranelift-lowering artifacts, not the full Moonlift backend contract.**  
...
- **C signed integer operators are a major semantic trap.**  
  Moonlift lowering currently uses wrapping integer semantics for ordinary arithmetic. Cranelift `iadd/isub/imul` are bitwise/wrapping unless extra flags are used. In C, signed overflow is UB. Emitting `int32_t a + b` for `BackI32` would not match Cranelift under overflow.
...
- **Pointer arithmetic in Cranelift is integer arithmetic; pointer arithmetic in C is provenance-constrained.**  
  The Rust decoder lowers `PtrOffset` as `base + index * elem_size + const_offset` on pointer-sized integers. In C, `p + n` is only defined inside one object, and integer-pointer roundtrips have provenance concerns. Generated C must not accidentally make Moonlift pointer math depend on C object-provenance rules.

- **Memory loads/stores cannot safely be lowered as typed pointer dereferences in general.**  
  A direct `*(int32_t*)addr` can violate C alignment, strict-aliasing, effective-type, or null-deref rules. Moonlift stack/data memory behaves more like byte-addressable storage; Cranelift loads/stores do not impose C effective-type rules.

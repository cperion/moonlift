# Moonlift Copy-Link-Patch compiler tier exploration 
Explore feasibility and architecture options for a copy-and-patch / copy-link-patch compiler tier in Moonlift, grounded in cp.txt and experiments/lua_interpreter_vm/SPONJIT_COPY_LINK_PATCH.md.
**Workflow ID**: wf-moonlift-clp
...
2. `experiments/lua_interpreter_vm/SPONJIT_MOONLIFT_COPY_PATCH_DESIGN.md` (lines 1-751) — Current SpongeJIT copy/patch executor design; actual expected data/control tree.
...
5. `experiments/lua_interpreter_vm/spongejit/ssa_asdl/spongejit_lua_ssa.asdl` (lines 1-80, 1580-2114) — ASDL schema for LuaSrc/LuaExec/MoonCFG/Stencil.
...
7. `experiments/lua_interpreter_vm/spongejit/lua_compile/lua_compile_foundry.lua` (1-580) — Maintained offline foundry: opcode windows + facts → MoonCFG + contract + Stencil.VariantKey representatives.
...
13. `experiments/lua_interpreter_vm/src/regions_value.lua` (1-286), `regions_table.lua` (1-674), `vm_loop.lua` (1-180), `api.lua` (1-174) — Existing Moonlift-native VM semantic/fact-observation substrate.
14. `experiments/lua_interpreter_vm/src/jit/stencil_codegen.lua` (1-70) — Early bootstrap stub for StateOp → Moonlift stencil source.
...
16. `lua/moonlift/back_jit.lua` (1-217), `hosted_jit.lua` (1-59), `back_object.lua` (1-88), `back_command_binary.lua` (1-840), `BACK_WIRE_FORMAT.md` (1-456) — Current Moonlift backend FFI/wire path.
...
18. `lua/moonlift/init.lua` (1-198), `frontend_pipeline.lua` (1-220), `tree_to_back.lua` (1-2730 sampled), `back_validate.lua` (1-970 sampled), `schema/back.lua` (520-950, 1385-1388) — Frontend-to-flat-backend pipeline and `BackProgram` schema.
19. `lua/moonlift/link_*.lua`, `emit_object.lua`, `emit_shared.lua`, `tests/test_back_object_emit.lua` — Current AOT object/shared emission and linking path.
...
`experiments/lua_interpreter_vm/SPONJIT_MOONLIFT_COPY_PATCH_DESIGN.md` defines current intended flow:
...
`lua_compile_foundry.lua` compiles source opcode windows and fact bundles through LuaCompile into `MoonCFG.Kernel`, then computes representative identity:
...
-> MoonCFG + kernel LuaContract + Stencil.VariantKey representative dedupe
-> MoonCFG/Moonlift source artifacts; binary StencilTemplate banks are separate object-extraction artifacts.
```

### Current Moonlift backend/AOT path

Lua side:
- `frontend_pipeline.parse_and_lower` parses/typechecks/lowers to `MoonBack.BackProgram`.
...
- `init.lua` exposes `moon.emit_object` and `moon.emit_shared`.
...
### Paper mechanics vs Moonlift/SpongeJIT state
...
- Current SpongeJIT:
  ...
  - Runtime executable allocation/protection/publish is design-only in `SPONJIT_MOONLIFT_COPY_PATCH_DESIGN.md`; not implemented in the current Lua materializer.

### MoonCFG/foundry to stencil identity
...
- `MoonCFG.emit` already produces Moonlift source for foundry kernels.
- `moon.emit_object` / `back_object.lua` can compile Moonlift source through Cranelift to relocatable object bytes.
...
- The user-mentioned file `experiments/lua_interpreter_vm/SPONJIT_COPY_LINK_PATCH.md` does not exist. The current matching document is `SPONJIT_MOONLIFT_COPY_PATCH_DESIGN.md`.
...
- Potential bug/ABI mismatch observed: `lua/moonlift/back_object.lua` declares `moonlift_bytes_free(uint8_t* data, size_t len)`, but current `src/ffi.rs` defines `moonlift_bytes_free(bytes: *mut moonlift_bytes_t)`. The old museum version had the two-argument signature.
- Old shadow prototype under `tools/sponjit_shadow` contains C stencils, hole externs, ELF extraction, and abstract stencil lowering, but comments/design vocabulary there are explicitly older and opcode/SSA-descriptor-flavored compared to current ASDL/MoonCFG direction.
...
- Moonlift already has an AOT object path, but that path emits **whole relocatable objects**, not extractable copy-and-patch stencils. The missing piece is not “getting bytes”; it is preserving a typed mapping from semantic patch sources to concrete object relocations/byte offsets.

- The paper’s MetaVar trick relies on placeholder extern symbols so the object file contains relocation records for holes. Ordinary Moonlift constants, stack offsets, branch destinations, and function references compiled through Cranelift may become opaque instruction bytes with no recoverable semantic identity.
...
- The paper’s register allocation is not generic CPS; it depends on the **GHC calling convention** and on tail calls becoming jumps. Moonlift/Cranelift’s normal C/System V-style ABI does not automatically provide the same pass-through register preservation semantics.

- Moonlift regions look CPS-like at the source/control level, but `emit` splices regions into CFGs before backend lowering. That is almost the inverse of paper-style binary stencil composition: Moonlift erases region boundaries for optimization, while copy-and-patch needs selected binary boundaries to survive as copyable artifacts.
...
- The current ASDL `Stencil` schema is richer than the Moonlift runtime design sketch: it includes FFI, GC, GOT/PLT, target-specific relocations, and many patch kinds. The simplified runtime patch vocabulary would reject or under-specify some artifacts that the maintained ASDL can describe.
...
- A stencil bank’s `target` identity cannot safely mean only architecture/triple. It is coupled to Cranelift version, optimization flags, PIC/code model, CPU features, pointer width, endianness, Moonlift ABI, Flatline wire version, ASDL schema version, and VM struct layout.
...
- The observed `moonlift_bytes_free` FFI mismatch is relevant beyond being a bug: it shows that the current Lua↔Rust object-emission seam is not yet a hardened artifact ABI. A copy-and-patch bank generator would be more sensitive to this kind of drift than normal object emission.
...
- Whether Moonlift can request or model a calling convention suitable for pass-through register stencils.
...
- **Core idea**: Keep Moonlift’s Flatline → Cranelift backend as the authority, but extend object/JIT emission to produce extractable native stencil artifacts with typed patch metadata.

- **Key changes**:
  - Extend `src/lib.rs`, `src/decode.rs`, and object emission to expose per-function `.text` bytes, symbols, relocations, and addends.
  - Extend `lua/moonlift/back_object.lua` / backend FFI with a “compile stencil artifact” entrypoint.
  - Teach SpongeJIT foundry flow to compile MoonCFG-emitted Moonlift source into Cranelift objects, then convert relocations into `StencilTemplate`.
...
- **Risk**: Cranelift may optimize away or obscure values that need to remain patch holes unless Moonlift deliberately emits placeholder symbols / externs that survive into relocations.

- **Rough sketch**:
  - Add explicit “hole extern” lowering in Moonlift-generated stencil source.
...
- **Core idea**: Move copy-link-patch up one level: specialize and patch Moonlift `BackProgram` / Flatline templates, then lower the patched artifact to either Cranelift native code or Wasm.
...
### Approach C: Moonlift-Owned Stencil Backend

- **Core idea**: Build a dedicated stencil codegen path owned by Moonlift/SpongeJIT, with explicit stencil ABI, register protocol, patch encodings, and optional native/Wasm emitters.

- **Key changes**:
  - Add a low-level `StencilIR` / `MachineStencil` layer distinct from normal `MoonBack.BackProgram`.
...
- **Rough sketch**:
  ...
  - Lower selected MoonCFG kernels to this stencil IR.

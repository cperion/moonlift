...
for k, v in pairs(const.Status) do VALS["THREAD_" .. k] = lalin.int(v) end
for k, v in pairs(const.Err) do VALS["ERR_" .. k] = lalin.int(v) end
for k, v in pairs(const.Abi) do VALS["ABI_" .. k] = lalin.int(v) end

-- API functions are Lalin funcs (not regions) — sealed external boundaries.

-- lua_type_api: return type code for value at index, or -1 for invalid indices.
local lua_type_api = lalin.func(VALS) [[
...
local lua_settop_api = lalin.func(VALS) [[
...
local lua_pushvalue_api = lalin.func(VALS) [[
...
local lua_tolstring_api = lalin.func(VALS) [[
...
local lua_vm_abi_version_api = lalin.func(VALS) [[
lua_vm_abi_version_api(): i32
    return @{ABI_VM_VERSION}
end
]]

local lua_native_abi_version_api = lalin.func(VALS) [[
lua_native_abi_version_api(): i32
    return @{ABI_NATIVE_VERSION}
end
]]

local lua_status_api = lalin.func(VALS) [[
...
local lua_last_error_api = lalin.func(VALS) [[
...
local lua_gettable_api = lalin.func(VALS) [[
...
local lua_settable_api = lalin.func(VALS) [[
...
local lua_call_api = lalin.func(VALS) [[
...
local lua_pcall_api = lalin.func(VALS) [[

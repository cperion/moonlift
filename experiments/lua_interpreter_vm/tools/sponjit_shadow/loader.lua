...
local function sorted_entries(map, max_len)
    ...
    for key, count in pairs(map or {}) do
        local ops = split_ops(key)
        if #ops > 0 and (not max_len or #ops <= max_len) then
            xs[#xs + 1] = { key = key, ops = ops, count = tonumber(count) or 0 }
        end
    end
    ...
    for _, e in ipairs(entries) do
        ...
        if not seen[e.key] then
            ...
            out[#out + 1] = {
                ...
                events = M.events_from_ops(e.ops, e.count, config),
            }
        end
    end
    return out
...
function M.load_awfy_profile(root)
    local awfy = require("tools.jit_harness.awfy")
...
function M.profile_lua_root(root, config)
    config = config or {}
    local puc = require("tools.jit_harness.puc_bytecode")

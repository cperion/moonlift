...
local T = Schema.get()
local CFG, CC, RT, Exec, Stencil = T.LalinCFG, T.CompileContract, T.LuaRT, T.LuaExec, T.Stencil

local function kernel_from(events, evidence)
  local r = C.compile_to_lalin_kernel(C.unit_from_events(events, evidence or {}))
  assert(r.kind == "Ok", "compile_to_lalin_kernel rejected fixture: " .. tostring(r.diagnostic and r.diagnostic.reason and r.diagnostic.reason.kind))
  assert(r.product.kernel.id.name.text == "lua_exec_core_kernel", "LalinKernel must be produced by LuaExec route")
...
local function compile_quote(kernel, name)
  local native, errors = Emit.compile(kernel, { name = name })
  assert(native, table.concat(errors or {}, "; "))
  return native
end
...
local function param(s, lalin_type) return CFG.Param(name(s), ty(lalin_type), CFG.ValueParam) end
...
-- Positive structural slice: LOADI + RETURN1 becomes a one-block LalinCFG kernel.
local kernel = kernel_from({ {op="LOADI",pc=1,a=1,b=42}, {op="RETURN1",pc=2,a=1} }, {})
assert(pvm.classof(kernel) == CFG.Kernel)
assert(kernel["normal" .. "_form"] == nil, "LalinCFG kernel must not carry retired executable payloads")
...
local src = Emit.emit(kernel, { name = "test_lalin_cfg_loadi" })
assert(src:match("local test_lalin_cfg_loadi = func"))
assert_no_protocol_source(src)
assert(src == Emit.emit(kernel, { name = "test_lalin_cfg_loadi" }), "LalinCFG emission must be deterministic")
local native = compile_quote(kernel, "test_lalin_cfg_loadi")
assert(native() == 42)
native:free()

-- ADDI scalar arithmetic is now LuaExec-owned. Because arithmetic has an
-- explicit nonnumeric error edge, the public LalinKernel route returns a typed
...
local add_src = Emit.emit(addk, { name = "test_lalin_cfg_addi" })
...
-- LalinCFG jump carrying the live scalar slot to the target block. The skipped
...
local closed_jump_native = compile_quote(closed_jump, "test_closed_cfg_jmp")
...
local closed_eqi_native = compile_quote(closed_eqi, "test_closed_cfg_eqi")
...
local closed_test_native = compile_quote(closed_test, "test_closed_cfg_test")
...
local closed_testset_native = compile_quote(closed_testset, "test_closed_cfg_testset")
...
local closed_branch_arg_native = compile_quote(closed_branch_arg, "test_closed_cfg_branch_arg")
...
for _, case in ipairs(rr_cases) do
  ...
  local n = compile_quote(k, "test_closed_cfg_rr_" .. case[1]:lower())
...
for _, case in ipairs(ri_cases) do
  ...
  assert_no_protocol_source(s)
  local n = compile_quote(k, "test_closed_cfg_ri_" .. case[1]:lower())
...
local eqk_native = compile_quote(eqk_kernel, "test_closed_cfg_eqk")
...
local external_jump = C.compile_to_lalin_kernel(C.unit_from_events({
  {op="JMP",pc=1,offset=99},
  {op="RETURN0",pc=2},
}, {}))
assert(external_jump.kind == "Reject", "external jump target must not compile as closed success")

local legacy_mul = C.compile_to_lalin_kernel(C.unit_from_events({
...
-- Manual LalinCFG: multi-block BoolChoice branch closes inside the kernel and
-- runs as native Lalin. This is positive CFG target coverage, not Lua opcode
-- semantic coverage.
...
local branch_src = Emit.emit(branch_kernel, { name = "test_lalin_cfg_bool_branch" })
assert(branch_src:match("return region %-%> i64"), "multi-block kernels should render as a Lalin region expression")
...
local branch_native = compile_quote(branch_kernel, "test_lalin_cfg_bool_branch")
assert(branch_native(true) == 11)
assert(branch_native(false) == 22)
branch_native:free()

-- Manual LalinCFG: Jump carries a named block parameter and returns it.
local x = param("x", "i64")
...
local jump_src = Emit.emit(jump_kernel, { name = "test_lalin_cfg_jump_param" })
...
local jump_native = compile_quote(jump_kernel, "test_lalin_cfg_jump_param")
assert(jump_native(99) == 99)
jump_native:free()

-- Manual LalinCFG: simple NumericChoice branch.
...
local numeric_src = Emit.emit(numeric_kernel, { name = "test_lalin_cfg_numeric_branch" })
...
local numeric_native = compile_quote(numeric_kernel, "test_lalin_cfg_numeric_branch")
...
local function assert_unsupported_reject(op, needle)
  local r = C.compile_to_lalin_kernel(C.unit_from_events({ sample_event(op), { op="RETURN0", pc=2 } }, {}))
...
assert(not ok and table.concat(errs, "\n"):match("unsupported_op"), "LalinCFG.EmitRegion must remain unsupported")
...
assert(not ok and table.concat(errs, "\n"):match("unsupported_terminator:Continue"), "LalinCFG.Continue must remain unsupported")
...
local multi_return_block = CFG.Block(bid, {}, {}, CFG.Return({ CFG.ConstValue(CFG.I64Const(1)), CFG.ConstValue(CFG.I64Const(2)) }))
local multi_return_region = CFG.Region(rid, {}, {}, bid, { multi_return_block })
local multi_return_kernel = CFG.Kernel(kid, CFG.InlineSpan, {}, { CFG.TypeRef("i64"), CFG.TypeRef("i64") }, multi_return_region, empty_contract)
local quote_native, quote_errors = Emit.compile(multi_return_kernel, { name = "test_quote_no_source_fallback" })
assert(not quote_native and table.concat(quote_errors or {}, "\n"):match("unsupported_kernel_return_arity"), "Emit.compile must fail closed instead of delegating unsupported quote paths to source compatibility")

print("ok - SpongeJIT LuaCompile LalinCFG LuaExec route")

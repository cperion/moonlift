...
assert(exec and pvm.classof(exec) == T.LuaExec.Kernel, "LuaExec lowering must own public LalinKernel fixture")
local mk = C.compile_to_lalin_kernel(unit)
assert(mk.kind == "Ok" and mk.product.kind == "LalinKernel")
assert(pvm.classof(mk.product.kernel) == T.LalinCFG.Kernel)
assert(pvm.classof(mk.product.kernel.contract) == T.CompileContract.Contract)
assert(mk.product.kernel.id.name.text == "lua_exec_core_kernel", "LalinKernel success must route through LuaExec")
assert(mk.product.kernel["normal" .. "_form"] == nil, "LalinCFG kernel must not carry retired executable payloads")
local mk2 = C.compile_to_lalin_kernel(unit)
assert(mk2.product.kernel == mk.product.kernel, "LalinKernel compile must return the same interned product for the same unit")
assert(LuaExecLower.lower(unit.source, unit.evidence) == exec)
local cfg = LuaExecToLalin.lower(exec)
assert(LuaExecToLalin.lower(exec) == cfg)
local outcome_cfg = LuaExecToLalin.lower(exec, { outcome = true, outcome_projection = "kind" })
assert(LuaExecToLalin.lower(exec, { outcome = true, outcome_projection = "kind" }) == outcome_cfg)

-- LuaCompile file manifest.  LalinCFG semantic execution is quote-first
-- (`lalin_cfg_quote_emit.lua` via `lalin_cfg_emit.compile/build/run`); the
-- hand-concatenating `lalin_cfg_emit_source_compat.lua` renderer is retained for
-- compatibility/debug serialization through `lalin_cfg_emit.emit` and foundry
-- source artifacts only.
local planned = {
"builders.lua", "compile_contract_key.lua", "compile_contract_validate.lua", "diagnostics.lua", "errors.lua", "init.lua", "lua_compile_foundry.lua", "lua_compile_to_lalin_kernel.lua", "lua_compile_unit.lua", "lua_compile_validate.lua", "lua_exec_region_model.lua", "lua_exec_static_region_inline.lua", "lua_exec_static_region_model.lua", "lua_exec_to_lalin_cfg_lower.lua", "lua_exec_validate.lua", "lua_fact_closure.lua", "lua_fact_contradiction.lua", "lua_fact_from_foundry_bundle.lua", "lua_fact_from_runtime_observe.lua", "lua_fact_payload_lease.lua", "lua_fact_validate.lua", "lua_ffi_validate.lua", "lua_gc_validate.lua", "lua_region_validate.lua", "lua_rt_arity_model.lua", "lua_rt_call_model.lua", "lua_rt_cdata_model.lua", "lua_rt_close_model.lua", "lua_rt_closure_upvalue_model.lua", "lua_rt_gc_alloc_model.lua", "lua_rt_loop_model.lua", "lua_rt_metatable_model.lua", "lua_rt_operation_model.lua", "lua_rt_object_model.lua", "lua_rt_outcome_model.lua", "lua_rt_stack_model.lua", "lua_rt_validate.lua", "lua_rt_value_model.lua", "lua_src_call_static_model.lua", "lua_src_closure_static_model.lua", "lua_src_from_puc_decode.lua", "lua_src_slot_alias.lua", "lua_src_to_lua_exec_lower.lua", "lua_src_to_lua_region_recognize.lua", "lua_src_validate.lua", "lua_src_window_collect.lua", "lalin_cfg_abi.lua", "lalin_cfg_emit.lua", "lalin_cfg_emit_source_compat.lua", "lalin_cfg_key.lua", "lalin_cfg_quote_emit.lua", "lalin_cfg_validate.lua", "schema.lua", "stencil_bank.lua", "stencil_bundle.lua", "stencil_foundry.lua", "stencil_key.lua", "stencil_manifest.lua", "stencil_materialization_plan.lua", "stencil_materialize.lua", "stencil_object_extract.lua", "stencil_validate.lua", "validate.lua" }
local seen = {}
...
for f in p:lines() do
  ...
  assert(not f:match("lalin" .. "_cfg_closed"), "retired file remains: " .. f)

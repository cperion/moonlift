...
local Exec, CFG = T.LuaExec, T.LalinCFG
...
local function compile_add(events, projection, name, evidence)
  ...
  assert(contains_class(cfg, CFG.RuntimeArithmeticNoMeta), "LalinCFG must contain explicit runtime arithmetic expression")
  assert(contains_class(cfg, CFG.RuntimeArithmeticNumericOk), "LalinCFG must contain explicit numeric-ok test")
  assert(contains_class(cfg, CFG.RuntimeArithmeticErrorValue), "LalinCFG must contain explicit arithmetic error value selection")
...
local public_addi = C.compile_to_lalin_kernel(C.unit_from_events(addi_window, {}))
...
local public_addk = C.compile_to_lalin_kernel(addk_unit)

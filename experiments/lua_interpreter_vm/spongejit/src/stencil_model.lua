...
local COVERS = {}
for _, s in ipairs(STENCILS) do
    for _, c in ipairs(s.covers or {}) do
        COVERS[c] = COVERS[c] or {}
        COVERS[c][#COVERS[c]+1] = s
    end
end
...
local function node_codegen_op(n) return n and (n.codegen_op or n.op) end
...
function M.lower(ssa_result)
    ...
    while i <= #active do
        local n = active[i]
        local op = node_codegen_op(n)
        local best = find_cover(op)
        if not best then
            return nil, "no stencil for SSA node " .. tostring(n.op) .. " / codegen " .. tostring(op)
        end
    ...
    for _, n in ipairs(active) do template.active_ops[#template.active_ops + 1] = node_codegen_op(n) end
    return template, nil
end
...
function M.template_from_ssa(ssa_result)
    local tmpl, err = M.lower(ssa_result)

-- lalin_cfg_quote_emit.lua -- quote-first LalinCFG -> Lalin semantic emitter.
--
-- This module is the primary home for semantic LalinCFG emission.  New lowering
-- must construct Lalin through lalin.* typed quotes/splices (lalin.func,
-- lalin.stmts, lalin.expr, lalin.type, lalin.params, lalin.fields, ...).  The legacy
-- hand-concatenating source renderer lives in lalin_cfg_emit_source_compat.lua
-- for compatibility/debug serialization only.

local lalin = require("lalin.host")
...
local function check_kernel(kernel)
  ...
  if not ok then
    return nil, errors or { "lalin_cfg_quote_emit:validation_failed" }
  end
  return true, nil
end
...
local function unsupported(kernel, why)
  ...
  return nil, { "lalin_cfg_quote_emit:not_yet_migrated:" .. tostring(name) .. (why and (":" .. tostring(why)) or "") }
end
...
local TY_VALUE = lalin.path_named(ValueModel.TYPE_NAME)
local TY_OUTCOME = lalin.path_named(OutcomeModel.TYPE_NAME)
local TY_SEQ = lalin.path_named(StackModel.SEQ_TYPE_NAME)
local TY_VARARG = lalin.path_named(StackModel.VARARG_TYPE_NAME)
local TY_CALL_FRAME = lalin.path_named(CallModel.FRAME_TYPE_NAME)
local TY_STRING = lalin.path_named(ObjectModel.STRING_TYPE_NAME)
local TY_TABLE = lalin.path_named(ObjectModel.TABLE_TYPE_NAME)
local TY_HASH_ENTRY = lalin.path_named(ObjectModel.HASH_ENTRY_TYPE_NAME)
local TY_RAW_GET = lalin.path_named(ObjectModel.RAW_GET_TYPE_NAME)
local TY_CDATA = lalin.path_named(CDataModel.TYPE_NAME)
local TY_VALUE_PTR = lalin.ptr(TY_VALUE)
local TY_STRING_PTR = lalin.ptr(TY_STRING)
local TY_TABLE_PTR = lalin.ptr(TY_TABLE)
local TY_HASH_ENTRY_PTR = lalin.ptr(TY_HASH_ENTRY)
local TY_I64_PTR = lalin.ptr(lalin.i64)

local function type_from_lalin_string(s)
  s = tostring(s or "i64")
  if s == "void" then return lalin.void
  elseif s == "bool" then return lalin.bool
  elseif s == "i8" then return lalin.i8
  elseif s == "i16" then return lalin.i16
  elseif s == "i32" then return lalin.i32
  elseif s == "i64" then return lalin.i64
  elseif s == "u8" then return lalin.u8
  elseif s == "u16" then return lalin.u16
  elseif s == "u32" then return lalin.u32
  elseif s == "u64" then return lalin.u64
  elseif s == "f32" then return lalin.f32
  elseif s == "f64" then return lalin.f64
  elseif s == "index" then return lalin.index
  ...
  if inner then return lalin.ptr(type_from_lalin_string(inner)) end
  return lalin.path_named(s)
end

local function type_from_ref(ref)
  return type_from_lalin_string(ref and ref.lalin_type or ref)
end

local function q_name(name, ty)
  assert(type(name) == "string" and name:match("^[_%a][_%w]*$"), "q_name expects an identifier")
  local HT = lalin.default_session.T
  local Tr, Bind = HT.LalinTree, HT.LalinBind
  local tv = ty and lalin.as_type_value(ty, "q_name type must be a Lalin type") or nil
  return lalin.expr_from_asdl(Tr.ExprRef(Tr.ExprSurface, Bind.ValueRefName(name)), tv, name, { ref_name = name })
end

local function q_i64(v) return lalin.int(math.floor(n(v))):as(lalin.i64) end
local function q_index(v) return lalin.int(math.floor(n(v))):as(lalin.index) end
local function q_f64(v) return lalin.float(tonumber(v) or 0, lalin.f64) end
local function q_bool(v) return lalin.bool_lit(v and true or false) end
local function q_null_value_ptr() return lalin.expr[[as(ptr(LuaRTValue), as(i64, 0))]] end
...
local function nil_value()
  return lalin.expr{ tag = tag_value("NilTag") }[[
    LuaRTValue { tag = @{tag}, payload_i64 = 0, payload_f64 = 0.0 }
  ]]
end

local function rt_value(tag, payload_i64, payload_f64)
  return lalin.expr{ tag = tag, payload_i64 = payload_i64 or q_i64(0), payload_f64 = payload_f64 or q_f64(0) }[[
...
local function seq_value_at(seq, index)
  ...
  local inline
  if index == 0 then
    inline = lalin.expr{seq = seq}[[@{seq}.value0]]
  elseif index == 1 then
    inline = lalin.expr{seq = seq}[[@{seq}.value1]]
  else
    local idx = q_i64(index)
    local buffer = lalin.expr{seq = seq}[[@{seq}.buffer]]
    local base = lalin.expr{seq = seq}[[@{seq}.base]]
    inline = buffer:index((base + idx):as(lalin.index))
  end
  if index >= 2 then
    return lalin.expr{ idx = q_i64(index), seq = seq, fallback = nil_value(), inline = inline }[[
...
local function buffer_value_at(buffer, base, count, index)
  local idx = q_i64(index)
  local elem = buffer:index((base + idx):as(lalin.index))
  return lalin.expr{ idx = idx, buffer = buffer, count = count, fallback = nil_value(), elem = elem }[[
...
local function is_string_expr(v)
  return lalin.expr{ v = v, short_tag = tag_value("ShortStringTag"), long_tag = tag_value("LongStringTag") }[[@{v}.tag == @{short_tag} or @{v}.tag == @{long_tag}]]
end

local function numeric_kind_safe(strings, value)
  return lalin.expr{ strings = strings, v = value, int_tag = tag_value("IntegerTag"), float_tag = tag_value("FloatTag"), is_string = is_string_expr(value), dec_i = string_numeric_kind_value("DecimalInteger"), dec_f = string_numeric_kind_value("DecimalFloat") }[[
...
local function numeric_i64_safe(strings, value)
  return lalin.expr{ strings = strings, v = value, int_tag = tag_value("IntegerTag"), is_string = is_string_expr(value), dec_i = string_numeric_kind_value("DecimalInteger") }[[
...
local function arithmetic_numeric_ok(op, strings, left, right)
  ...
  return lalin.expr{ lk = lk, rk = rk }[[@{lk} ~= as(i64, 0) and @{rk} ~= as(i64, 0)]]
end

local function arithmetic_error_value(op, _strings, left, right)
  if not op or op.kind ~= "ArithAdd" then return nil, "unsupported_arithmetic_op:" .. tostring(op and op.kind) end
  local left_prim = lalin.expr{ l = left, int_tag = tag_value("IntegerTag"), float_tag = tag_value("FloatTag") }[[@{l}.tag == @{int_tag} or @{l}.tag == @{float_tag}]]
  return lalin.expr{ lp = left_prim, l = left, r = right }[[
...
local function arithmetic_no_meta(op, strings, left, right)
  if not op or op.kind ~= "ArithAdd" then return nil, "unsupported_arithmetic_op:" .. tostring(op and op.kind) end
  local lk, rk = numeric_kind_safe(strings, left), numeric_kind_safe(strings, right)
  local both_int = lalin.expr{ lk = lk, rk = rk }[[@{lk} == as(i64, 1) and @{rk} == as(i64, 1)]]
  local numeric = lalin.expr{ lk = lk, rk = rk }[[@{lk} ~= as(i64, 0) and @{rk} ~= as(i64, 0)]]
  ...
  return lalin.expr{ both_int = both_int, numeric = numeric, int_tag = tag_value("IntegerTag"), float_tag = tag_value("FloatTag"), nil_tag = tag_value("NilTag"), int_sum = int_sum, float_sum = float_sum }[[
...
local function is_tag_expr(v, tag_name)
  return lalin.expr{ v = v, tag = tag_value(tag_name) }[[@{v}.tag == @{tag}]]
end

local function is_hash_key_expr(v)
  return lalin.expr{ v = v, int_tag = tag_value("IntegerTag"), short_tag = tag_value("ShortStringTag"), long_tag = tag_value("LongStringTag") }[[
...
local function collectable_expr(v)
  ...
  for _, t in ipairs(tags) do cond = lalin.expr{ prev = cond, v = v, tag = tag_value(t) }[[@{prev} or @{v}.tag == @{tag}]] end
  return cond
end

local function hash_key_equal_expr(entry_key, key)
  return lalin.expr{ e = entry_key, k = key, int_tag = tag_value("IntegerTag"), short_tag = tag_value("ShortStringTag"), long_tag = tag_value("LongStringTag") }[[
...
local function raw_get_initial()
  return lalin.expr{ absent_tag = tag_value("AbsentKeyTag"), absent_payload = q_i64(ValueModel.payload_for_nil_kind(RT.AbsentKeySentinel)) }[[
...
local function emit_raw_get_let(builder, env, dst, expr_node)
  ...
  env[dst] = dst_ref
  local done = builder:var(dst .. "_done", lalin.bool, q_bool(false))
  builder:if_(lalin.expr{ tv = tablev, tag = tag_value("TableTag") }[[@{tv}.tag == @{tag} and @{tv}.payload_i64 >= as(i64, 0)]], function(b)
    local tidx = b:let(dst .. "_table_index", lalin.index, tablev:field("payload_i64", lalin.i64):as(lalin.index))
    local tbl = tables:index(tidx)
    b:if_(tbl:field("metatable_kind", lalin.i64):eq(metatable_kind_value("NoMetatable")), function(nb)
      nb:if_(lalin.expr{ k = key, int_tag = tag_value("IntegerTag"), tbl = tbl }[[@{k}.tag == @{int_tag} and @{k}.payload_i64 >= as(i64, 1) and @{k}.payload_i64 <= @{tbl}.array_len and @{tbl}.array ~= as(ptr(LuaRTValue), as(i64, 0))]], function(ab)
        ab:set(dst_ref:place():field("hit", lalin.bool), q_bool(true))
        set_lua_value(ab, dst_ref:place():field("value", TY_VALUE), tbl:field("array", TY_VALUE_PTR):index((key:field("payload_i64", lalin.i64) - q_i64(1)):as(lalin.index)))
        ab:set(done:place(), q_bool(true))
      end)
      nb:if_(lalin.expr{ done = done, k = key, tbl = tbl, is_hash = is_hash_key_expr(key), limit = q_i64(ObjectModel.HASH_PROBE_LIMIT) }[[not @{done} and @{is_hash} and @{tbl}.hash_capacity > as(i64, 0) and @{tbl}.hash_capacity <= @{limit} and @{tbl}.hash ~= as(ptr(LuaRTTableHashEntry), as(i64, 0))]], function(hb)
        local hash = hb:let(dst .. "_hash", TY_HASH_ENTRY_PTR, tbl:field("hash", TY_HASH_ENTRY_PTR))
        for i = 0, ObjectModel.HASH_PROBE_LIMIT - 1 do
          hb:if_(lalin.expr{ done = done, tbl = tbl, i = q_i64(i) }[[not @{done} and @{i} < @{tbl}.hash_capacity]], function(pb)
            local entry = hash:index(q_index(i))
            pb:if_(lalin.expr{ e = entry, occupied = hash_state_value("Occupied"), eq = hash_key_equal_expr(entry:field("key", TY_VALUE), key) }[[@{e}.state == @{occupied} and @{eq}]], function(eb)
              eb:set(dst_ref:place():field("hit", lalin.bool), q_bool(true))
      ...
      nb:if_(lalin.expr{ done = done }[[not @{done}]], function(mb)
        mb:set(dst_ref:place():field("hit", lalin.bool), q_bool(true))
...
local function emit_raw_set_can_write_let(builder, env, dst, expr_node)
  local tables, tablev, key = value_to_expr(expr_node.tables, env), value_to_expr(expr_node.table_value, env), value_to_expr(expr_node.key, env)
  local out = builder:var(dst, lalin.bool, q_bool(false)); env[dst] = out
  builder:if_(lalin.expr{ tv = tablev, tag = tag_value("TableTag") }[[@{tv}.tag == @{tag} and @{tv}.payload_i64 >= as(i64, 0)]], function(b)
    local tidx = b:let(dst .. "_table_index", lalin.index, tablev:field("payload_i64", lalin.i64):as(lalin.index))
    local tbl = tables:index(tidx)
    b:if_(tbl:field("metatable_kind", lalin.i64):eq(metatable_kind_value("NoMetatable")), function(nb)
      nb:if_(lalin.expr{ k = key, int_tag = tag_value("IntegerTag"), tbl = tbl }[[@{k}.tag == @{int_tag} and @{k}.payload_i64 >= as(i64, 1) and @{k}.payload_i64 <= @{tbl}.array_len and @{tbl}.array ~= as(ptr(LuaRTValue), as(i64, 0))]], function(ab) ab:set(out:place(), q_bool(true)) end)
      nb:if_(lalin.expr{ out = out, k = key, tbl = tbl, is_hash = is_hash_key_expr(key), limit = q_i64(ObjectModel.HASH_PROBE_LIMIT) }[[not @{out} and @{is_hash} and @{tbl}.hash_capacity > as(i64, 0) and @{tbl}.hash_capacity <= @{limit} and @{tbl}.hash ~= as(ptr(LuaRTTableHashEntry), as(i64, 0))]], function(hb)
        local hash = hb:let(dst .. "_hash", TY_HASH_ENTRY_PTR, tbl:field("hash", TY_HASH_ENTRY_PTR))
        for i = 0, ObjectModel.HASH_PROBE_LIMIT - 1 do
          hb:if_(lalin.expr{ out = out, tbl = tbl, i = q_i64(i) }[[not @{out} and @{i} < @{tbl}.hash_capacity]], function(pb)
            local entry = hash:index(q_index(i))
            pb:if_(lalin.expr{ e = entry, empty = hash_state_value("Empty"), tomb = hash_state_value("Tombstone"), occupied = hash_state_value("Occupied"), eq = hash_key_equal_expr(entry:field("key", TY_VALUE), key) }[[
...
local function table_barrier_needed_expr(tables, tablev, value)
  return lalin.expr{ tables = tables, tv = tablev, v = value, table_tag = tag_value("TableTag"), black = gc_color_value("Black"), old = gc_generation_value("Old"), collect = collectable_expr(value) }[[
...
local function table_array_len_expr(tables, tablev)
  return lalin.expr{ tables = tables, tv = tablev, table_tag = tag_value("TableTag") }[[
...
local function string_len_expr(strings, value)
  return lalin.expr{ strings = strings, v = value, is_str = is_string_expr(value) }[[
...
local function len_no_meta_ok_expr(strings, tables, value)
  return lalin.expr{ tables = tables, v = value, table_tag = tag_value("TableTag"), is_str = is_string_expr(value), no_meta = metatable_kind_value("NoMetatable") }[[
...
local function len_no_meta_expr(strings, tables, value)
  return lalin.expr{ strings = strings, tables = tables, v = value, table_tag = tag_value("TableTag"), is_str = is_string_expr(value), no_meta = metatable_kind_value("NoMetatable"), int_tag = tag_value("IntegerTag") }[[
...
local function concat2_expr(strings, left, right)
  return lalin.expr{ strings = strings, l = left, r = right, is_l = is_string_expr(left), is_r = is_string_expr(right), tag = tag_value("LongStringTag"), nil_tag = tag_value("NilTag") }[[
...
value_to_expr = function(v, env)
  ...
  elseif c == CFG.ConstValue then
    ...
    elseif k == CFG.StringConst then return lalin.string_lit(v.const.value or "") end
  elseif c == CFG.UnitValue then
    return lalin.nil_lit()
  end
  error("lalin_cfg_quote_emit unsupported LalinCFG.Value: " .. tostring(v and v.kind), 2)
end
...
local function value_to_index_expr(v, env)
  ...
  return value_to_expr(v, env):as(lalin.index)
end
...
local function quote_runtime_expr(e, env)
  ...
  elseif c == CFG.RuntimeBoxBool then
    ...
    return lalin.expr{ b = b, true_tag = tag_value("TrueTag"), false_tag = tag_value("FalseTag") }[[
  ...
  elseif c == CFG.RuntimeTag then return lalin.expr{ v = value_to_expr(e.value, env) }[[@{v}.tag]]
  elseif c == CFG.RuntimePayloadI64 then return lalin.expr{ v = value_to_expr(e.value, env) }[[@{v}.payload_i64]]
  elseif c == CFG.RuntimePayloadF64 then return lalin.expr{ v = value_to_expr(e.value, env) }[[@{v}.payload_f64]]
  ...
  elseif c == CFG.Primitive then
    ...
    elseif k == "Not" then return lalin.expr{ a = a }[[not @{a}]]
  ...
  elseif c == CFG.AddressOf then
    ...
    return lalin.addr_of(p)
  elseif c == CFG.RuntimeTruthiness then
    local v = value_to_expr(e.value, env)
    return lalin.expr{ v = v, nil_tag = tag_value("NilTag"), empty_tag = tag_value("EmptySlotTag"), absent_tag = tag_value("AbsentKeyTag"), no_table_tag = tag_value("NoTableTag"), false_tag = tag_value("FalseTag") }[[
  ...
  elseif c == CFG.RuntimeTypeTest then
    ...
    for _, tag in ipairs(tags) do
      cond = lalin.expr{ prev = cond, v = v, tag = tag_value(tag) }[[@{prev} or @{v}.tag == @{tag}]]
    end
    return cond
  elseif c == CFG.RuntimeOutcomeReturn then
    return lalin.expr{ kind = outcome_kind_value("NormalReturnOutcome"), count = value_to_expr(e.count, env), v0 = outcome_value(e.values, 1, env), v1 = outcome_value(e.values, 2, env), nilv = nil_value(), nullp = q_null_value_ptr() }[[
  ...
  elseif c == CFG.RuntimeOutcomeReturnSeq then
    ...
    return lalin.expr{ kind = outcome_kind_value("NormalReturnOutcome"), seq = seq, v0 = lalin.expr{seq=seq}[[@{seq}.value0]], v1 = lalin.expr{seq=seq}[[@{seq}.value1]], nilv = nil_value() }[[
  ...
  elseif c == CFG.RuntimeOutcomeError then
    return lalin.expr{ kind = outcome_kind_value("LuaErrorOutcome"), ek = error_kind_value(e.kind), errv = value_to_expr(e.error_value, env), pc = value_to_expr(e.saved_pc, env), top = value_to_expr(e.saved_top, env), nilv = nil_value(), nullp = q_null_value_ptr() }[[
  ...
  elseif c == CFG.RuntimeOutcomeYield or c == CFG.RuntimeOutcomeYieldSeq then
    local count = c == CFG.RuntimeOutcomeYield and value_to_expr(e.count, env) or lalin.expr{seq=value_to_expr(e.seq, env)}[[@{seq}.count]]
    ...
    local buffer = c == CFG.RuntimeOutcomeYield and q_null_value_ptr() or lalin.expr{seq=value_to_expr(e.seq, env)}[[@{seq}.buffer]]
    local base = c == CFG.RuntimeOutcomeYield and q_i64(0) or lalin.expr{seq=value_to_expr(e.seq, env)}[[@{seq}.base]]
    return lalin.expr{ kind = outcome_kind_value("LuaYieldOutcome"), yk = yield_kind_value(e.resume_point), count = count, v0 = v0, v1 = v1, pc = value_to_expr(e.saved_pc, env), top = value_to_expr(e.saved_top, env), nilv = nil_value(), buffer = buffer, base = base }[[
  ...
  elseif c == CFG.RuntimeTopLoad then return lalin.expr{ p = value_to_expr(e.top_ptr, env) }[=[@{p}[as(index, 0)]]=]
  ...
  elseif c == CFG.RuntimeValueSeqFromStack then
    ...
    local b0 = cls(e.base) == CFG.ConstValue and cls(e.base.const) == CFG.I64Const and q_index(e.base.const.value) or base:as(lalin.index)
    local b1 = cls(e.base) == CFG.ConstValue and cls(e.base.const) == CFG.I64Const and q_index(e.base.const.value + 1) or (base + q_i64(1)):as(lalin.index)
  ...
  elseif c == CFG.RuntimeValueSeqFromVarargs then
    ...
    local values = lalin.expr{ src = src }[[@{src}.values]]
  ...
  elseif c == CFG.RuntimeValueSeqAdjust or c == CFG.RuntimeValueSeqNormalize then
    ...
    local count = target ~= nil and q_i64(target) or lalin.expr{ seq = seq }[[@{seq}.count]]
    return quote_seq("AdjustedSeq", count, lalin.expr{seq=seq}[[@{seq}.value0]], lalin.expr{seq=seq}[[@{seq}.value1]], lalin.expr{seq=seq}[[@{seq}.buffer]], lalin.expr{seq=seq}[[@{seq}.base]])
  elseif c == CFG.RuntimeValueSeqCount then return lalin.expr{ seq = value_to_expr(e.seq, env) }[[@{seq}.count]]
  elseif c == CFG.RuntimeValueSeqValue then return seq_value_at(value_to_expr(e.seq, env), e.index)
  elseif c == CFG.RuntimeValueSeqBuffer then return lalin.expr{ seq = value_to_expr(e.seq, env) }[[@{seq}.buffer]]
  elseif c == CFG.RuntimeValueSeqBase then return lalin.expr{ seq = value_to_expr(e.seq, env) }[[@{seq}.base]]
  elseif c == CFG.RuntimeVarargSource then
    return lalin.expr{ kind = vararg_kind_value("HiddenFrameVarargs"), values = value_to_expr(e.values, env), count = value_to_expr(e.count, env), table_handle = value_to_expr(e.table_handle, env) }[[
      LuaRTVarargSource { kind = @{kind}, values = @{values}, count = @{count}, table_handle = @{table_handle} }
    ]]
  elseif c == CFG.RuntimeVarargCount then return lalin.expr{ src = value_to_expr(e.source, env) }[[@{src}.count]]
  elseif c == CFG.RuntimeClassifyCallee then
    local callee = value_to_expr(e.callee, env)
    local callee_tag = callee:field("tag", lalin.i64)
  ...
  elseif c == CFG.RuntimeCallTargetCheck then
    ...
    if cls(identity) == RT.LuaClosureTargetIdentity then
      return lalin.expr{ tag = callee:field("tag", lalin.i64), payload = callee:field("payload_i64", lalin.i64), lua_tag = tag_value("LuaClosureTag"), handle = q_i64(identity.closure_handle) }[[
  ...
  elseif c == CFG.RuntimeVarargGet then
    ...
    return lalin.expr{ src = src, key = key, nilv = nil_value(), int_tag = tag_value("IntegerTag") }[[
  ...
  elseif c == CFG.RuntimeOutcomeKind then return lalin.expr{ out = value_to_expr(e.outcome, env) }[[@{out}.kind]]
  elseif c == CFG.RuntimeOutcomeCount then return lalin.expr{ out = value_to_expr(e.outcome, env) }[[@{out}.count]]
  elseif c == CFG.RuntimeOutcomeValueTag then
    return lalin.expr{ v = (tonumber(e.index or 0) == 0 and lalin.expr{out=value_to_expr(e.outcome, env)}[[@{out}.value0]] or tonumber(e.index or 0) == 1 and lalin.expr{out=value_to_expr(e.outcome, env)}[[@{out}.value1]] or buffer_value_at(lalin.expr{out=value_to_expr(e.outcome, env)}[[@{out}.value_buffer]], lalin.expr{out=value_to_expr(e.outcome, env)}[[@{out}.value_base]], lalin.expr{out=value_to_expr(e.outcome, env)}[[@{out}.count]], tonumber(e.index or 0))) }[[@{v}.tag]]
  ...
  elseif c == CFG.RuntimeOutcomeErrorKind then return lalin.expr{ out = value_to_expr(e.outcome, env) }[[@{out}.error_kind]]
  elseif c == CFG.RuntimeOutcomeErrorValueTag then return lalin.expr{ out = value_to_expr(e.outcome, env) }[[@{out}.error_value.tag]]
  elseif c == CFG.RuntimeOutcomeErrorValuePayloadI64 then return lalin.expr{ out = value_to_expr(e.outcome, env) }[[@{out}.error_value.payload_i64]]
  elseif c == CFG.RuntimeOutcomeSavedPc then return lalin.expr{ out = value_to_expr(e.outcome, env) }[[@{out}.saved_pc]]
  elseif c == CFG.RuntimeOutcomeYieldKind then return lalin.expr{ out = value_to_expr(e.outcome, env) }[[@{out}.yield_kind]]
  elseif c == CFG.RuntimeCallFramePrepare then
    return lalin.expr{ caller = value_to_expr(e.caller_stack, env), callee = value_to_expr(e.callee_stack, env), arg_base = q_i64(slot_index(e.layout and e.layout.arg_base)), arg_count = q_i64(fixed_count_spec_value(e.layout and e.layout.arg_count)), result_base = q_i64(slot_index(e.layout and e.layout.result_base)), result_count = q_i64(fixed_count_spec_value(e.layout and e.layout.result_count)) }[[
  ...
  elseif c == CFG.RuntimeRawGetHit then
    return value_to_expr(e.rawget, env):field("hit", lalin.bool)
  ...
  elseif c == CFG.RuntimeRawGetValueOrNil then
    ...
    return rg:field("hit", lalin.bool):select(rg:field("value", TY_VALUE), nil_value())
...
local function infer_expr_type(e)
  ...
  if c == CFG.Primitive then
    local k = e.op and e.op.kind
    if k == "DivF64" then return lalin.f64 end
    if k == "Eq" or k == "Lt" or k == "Le" or k == "Not" or k == "Truthy" then return lalin.bool end
    return lalin.i64
...
set_lua_value = function(builder, dst_place, src_expr)
  builder:set(dst_place:field("tag", lalin.i64), lalin.expr{ v = src_expr }[[@{v}.tag]])
  builder:set(dst_place:field("payload_i64", lalin.i64), lalin.expr{ v = src_expr }[[@{v}.payload_i64]])
  builder:set(dst_place:field("payload_f64", lalin.f64), lalin.expr{ v = src_expr }[[@{v}.payload_f64]])
end
...
local function emit_seq_value_let(builder, env, dst, expr_node)
  ...
  builder:if_(lalin.expr{ idx = idx, seq = seq }[[@{idx} < @{seq}.count]], function(b)
    if idxn == 0 then
      set_lua_value(b, dst_place, lalin.expr{ seq = seq }[[@{seq}.value0]])
    elseif idxn == 1 then
      set_lua_value(b, dst_place, lalin.expr{ seq = seq }[[@{seq}.value1]])
    else
      local buffer = lalin.expr{ seq = seq }[[@{seq}.buffer]]
      local base = lalin.expr{ seq = seq }[[@{seq}.base]]
      b:if_(lalin.expr{ buffer = buffer }[[@{buffer} ~= as(ptr(LuaRTValue), as(i64, 0))]], function(bb)
        set_lua_value(bb, dst_place, buffer:index((base + idx):as(lalin.index)))
      end)
    end
  end)
  return true
...
local function vararg_source_initial()
  return lalin.expr{ kind = vararg_kind_value("HiddenFrameVarargs"), nullp = q_null_value_ptr() }[[
...
local function emit_vararg_source_let(_builder, env, dst, expr_node)
  ...
  -- expression to avoid Lalin aggregate local initializer lowering limits.
...
local function emit_vararg_get_let(builder, env, dst, expr_node)
  ...
  local dst_ref = builder:var(dst, TY_VALUE, nil_value())
  env[dst] = dst_ref
  builder:if_(lalin.expr{ key = key, int_tag = tag_value("IntegerTag") }[[@{key}.tag == @{int_tag}]], function(ib)
    ib:if_(lalin.expr{ key = key }[[@{key}.payload_i64 >= as(i64, 1)]], function(lb)
      lb:if_(lalin.expr{ key = key, src = src }[[@{key}.payload_i64 <= @{src}.count]], function(cb)
        set_lua_value(cb, dst_ref:place(), src:field("values", TY_VALUE_PTR):index((key:field("payload_i64", lalin.i64) - q_i64(1)):as(lalin.index)))
      end)
    end)
  end)
  return true
...
local function emit_let(builder, env, op)
  local dst = assert(place_key(op.dst), "Let destination must be a named LalinCFG place")
...
local function seq_expr_at(seq, index)
  index = tonumber(index) or 0
  if index == 0 then return lalin.expr{ seq = seq }[[@{seq}.value0]] end
  if index == 1 then return lalin.expr{ seq = seq }[[@{seq}.value1]] end
  local idx = q_i64(index)
  local buffer = lalin.expr{ seq = seq }[[@{seq}.buffer]]
  local base = lalin.expr{ seq = seq }[[@{seq}.base]]
  return buffer:index((base + idx):as(lalin.index))
end
...
local function emit_seq_store(builder, stack, base, seq, start_index, max_count, base_const)
  ...
  for i = start_index, max_count - 1 do
    local idx = q_i64(i)
    builder:if_(lalin.expr{ seq = seq, idx = idx }[[@{seq}.count >= (@{idx} + as(i64, 1))]], function(b)
      local index = base_const ~= nil and q_index(base_const + i) or (base + idx):as(lalin.index)
...
local function emit_op(builder, env, op)
  ...
  elseif c == CFG.RuntimeTableRawSet then
    ...
    builder:if_(lalin.expr{ tv = tablev, tag = tag_value("TableTag") }[[@{tv}.tag == @{tag} and @{tv}.payload_i64 >= as(i64, 0)]], function(b)
      local tidx = b:let("raw_set_table_index", lalin.index, tablev:field("payload_i64", lalin.i64):as(lalin.index))
      local tbl = tables:index(tidx)
      local tbl_place = index_place_of(tables, tidx, TY_TABLE)
      local array_slot = b:var("raw_set_array_slot", lalin.bool, q_bool(false))
      b:if_(lalin.expr{ k = key, int_tag = tag_value("IntegerTag"), tbl = tbl }[[@{k}.tag == @{int_tag} and @{k}.payload_i64 >= as(i64, 1) and @{k}.payload_i64 <= @{tbl}.array_len]], function(ab)
        ab:set(array_slot:place(), q_bool(true))
        ab:if_(lalin.expr{ tbl = tbl }[[@{tbl}.array ~= as(ptr(LuaRTValue), as(i64, 0))]], function(sb)
          set_lua_value(sb, index_place_of(tbl:field("array", TY_VALUE_PTR), (key:field("payload_i64", lalin.i64) - q_i64(1)):as(lalin.index), TY_VALUE), value)
        end)
      end)
      b:if_(lalin.expr{ array_slot = array_slot, is_hash = is_hash_key_expr(key), tbl = tbl, limit = q_i64(ObjectModel.HASH_PROBE_LIMIT) }[[not @{array_slot} and @{is_hash} and @{tbl}.hash_capacity > as(i64, 0) and @{tbl}.hash_capacity <= @{limit} and @{tbl}.hash ~= as(ptr(LuaRTTableHashEntry), as(i64, 0))]], function(hb)
        local hash = hb:let("raw_set_hash", TY_HASH_ENTRY_PTR, tbl:field("hash", TY_HASH_ENTRY_PTR))
        local done = hb:var("raw_set_hash_done", lalin.bool, q_bool(false))
        for i = 0, ObjectModel.HASH_PROBE_LIMIT - 1 do
          hb:if_(lalin.expr{ done = done, tbl = tbl, i = q_i64(i) }[[not @{done} and @{i} < @{tbl}.hash_capacity]], function(ub)
            local entry = hash:index(q_index(i))
            local entry_place = index_place_of(hash, q_index(i), TY_HASH_ENTRY)
            ub:if_(lalin.expr{ e = entry, occupied = hash_state_value("Occupied"), eq = hash_key_equal_expr(entry:field("key", TY_VALUE), key) }[[@{e}.state == @{occupied} and @{eq}]], function(eb)
        ...
        for i = 0, ObjectModel.HASH_PROBE_LIMIT - 1 do
          hb:if_(lalin.expr{ done = done, tbl = tbl, i = q_i64(i) }[[not @{done} and @{i} < @{tbl}.hash_capacity]], function(ib)
            local entry = hash:index(q_index(i))
            local entry_place = index_place_of(hash, q_index(i), TY_HASH_ENTRY)
            ib:if_(lalin.expr{ e = entry, empty = hash_state_value("Empty"), tomb = hash_state_value("Tombstone") }[[@{e}.state == @{empty} or @{e}.state == @{tomb}]], function(eb)
              eb:set(entry_place:field("state", lalin.i64), hash_state_value("Occupied"))
              set_lua_value(eb, entry_place:field("key", TY_VALUE), key)
              set_lua_value(eb, entry_place:field("value", TY_VALUE), value)
              eb:set(tbl_place:field("hash_count", lalin.i64), tbl:field("hash_count", lalin.i64) + q_i64(1))
              eb:set(done:place(), q_bool(true))
            end)
          end)
        end
  ...
  elseif c == CFG.RuntimeTableWriteBarrier then
    ...
    builder:if_(table_barrier_needed_expr(tables, tablev, value), function(b)
      b:if_(lalin.expr{ tv = tablev, tag = tag_value("TableTag") }[[@{tv}.tag == @{tag} and @{tv}.payload_i64 >= as(i64, 0)]], function(tb)
        local idx = tb:let("barrier_table_index", lalin.index, tablev:field("payload_i64", lalin.i64):as(lalin.index))
        local tbl = tables:index(idx)
        local tbl_place = index_place_of(tables, idx, TY_TABLE)
        tb:set(tbl_place:field("barrier_count", lalin.i64), tbl:field("barrier_count", lalin.i64) + q_i64(1))
        tb:set(tbl_place:field("barrier_epoch", lalin.i64), tbl:field("gc_epoch", lalin.i64))
        tb:set(tbl_place:field("barrier_last_child_tag", lalin.i64), value:field("tag", lalin.i64))
        tb:set(tbl_place:field("barrier_last_child_payload", lalin.i64), value:field("payload_i64", lalin.i64))
      end)
    end)
    return true
...
local function return_type(kernel)
  ...
  if #returns == 0 then return lalin.void end
...
local function default_value_for_type(ty)
  ...
  if ty == lalin.void then return nil end
  if ty == lalin.bool then return q_bool(false) end
  if ty == lalin.f64 or ts == "f64" then return q_f64(0) end
  ...
  if ts == OutcomeModel.TYPE_NAME or ts:match(OutcomeModel.TYPE_NAME) then
    return lalin.expr{ kind = outcome_kind_value("NormalReturnOutcome"), nilv = nil_value(), nullp = q_null_value_ptr() }[[
...
local function tree_api()
  local S = lalin.default_session.T
  return S.LalinCore, S.LalinBind, S.LalinTree
end

local function as_expr(v, site) return lalin.as_lalin_expr(v, site or "quote expr") end
local function as_place(v, site) return lalin.as_lalin_place(v, site or "quote place") end
local function as_type_asdl(ty, site) return lalin.as_type_value(ty, site or "quote type").ty end

index_place_of = function(base_expr, index_expr, elem_ty)
  local S = lalin.default_session.T
  local Tr, Ty, Core = S.LalinTree, S.LalinType, S.LalinCore
  ...
  return lalin.place_from_asdl(place, elem_ty, "quote_index_place")
end
...
local function expr_ref_for_binding(binding, ty, name)
  return lalin.expr_ref(binding, ty, name)
end
...
local function quote_choice(choice, env)
  ...
  elseif c == CFG.NilChoice then
    ...
    return lalin.expr{ v = v, nil_tag = tag_value("NilTag") }[[@{v}.tag == @{nil_tag}]]
  ...
  elseif c == CFG.NumericChoice then
    ...
    elseif k == "NumNonZero" then return lalin.expr{ l = l }[[@{l} ~= as(i64, 0)]] end
...
local function append_terminator(builder, env, term, ret_ty, as_region)
  ...
  if c == CFG.Return then
    ...
    if ret_ty == lalin.void and #values == 0 then append_yield_or_return(builder, nil, as_region); return true end
  ...
  elseif c == CFG.Unreachable then
    append_yield_or_return(builder, ret_ty == lalin.void and nil or default_value_for_type(ret_ty), as_region)
    return true
  end
...
local function build_region_block(block, is_entry, region_params, base_env, ret_ty)
  ...
  if is_entry then
    ...
    params = lalin.entry_params(params)
  else
    local Tr = lalin.default_session.T.LalinTree
    for _, p in ipairs(block.params or {}) do
      local name, ty = render_name(p.name), param_type(p)
      params[#params + 1] = Tr.BlockParam(name, lalin.as_type_value(ty, "block param type").ty)
      env[name] = q_name(name, ty)
    end
  end
...
local function build_control_expr(kernel, ret_ty, base_env)
  ...
  local ty = lalin.as_type_value(ret_ty, "control expr result type").ty
  return lalin.expr_from_asdl(Tr.ExprControl(Tr.ExprSurface, Tr.ControlExprRegion(render_name(region.id and region.id.name), ty, entry_block, blocks)), ret_ty, "quote_control_region"), nil, "expr"
end
...
local function build_func_in_bundle(bundle, kernel, opts)
  ...
  -- Lalin's typed AST helpers because quote syntax cannot ergonomically splice
  -- arbitrary entry/block parameter binder lists.  All expression and runtime
  -- fragments remain lalin.* typed quote/splice values.
  local func = bundle:export_func(fname, lalin.params(params), ret_ty, function(builder)
    local base_env = {}
    ...
    if #blocks == 1 and cls(blocks[1].terminator) == CFG.Return and #(blocks[1].params or {}) == 0 then
      ...
      for _, op in ipairs(blocks[1].ops or {}) do
        ...
        if not done then error("lalin_cfg_quote_emit:not_yet_migrated:" .. tostring(err), 2) end
      end
      local values = blocks[1].terminator.values or {}
      if #values == 0 then return nil end
      if #values > 1 then error("lalin_cfg_quote_emit:not_yet_migrated:multi_value_return", 2) end
      return value_to_expr(values[1], env)
    end
    local ctrl, cerr, mode = build_control_expr(kernel, ret_ty, base_env)
    if not ctrl then error("lalin_cfg_quote_emit:not_yet_migrated:" .. tostring(cerr), 2) end
...
function M.build_func(kernel, opts)
  ...
  local bundle = lalin.module(name .. "_func_bundle")
...
function declare_runtime_abi(bundle)
  ...
  -- with lalin.fields typed quotes so the quote emitter owns the semantic path.
  bundle:struct(ValueModel.TYPE_NAME, lalin.fields[[
    tag: i64;
    payload_i64: i64;
    payload_f64: f64
  ]])
  bundle:struct(OutcomeModel.TYPE_NAME, lalin.fields[[
    kind: i64;
    count: i64;
    value0: LuaRTValue;
    value1: LuaRTValue;
  ...
  bundle:struct(StackModel.STACK_TYPE_NAME, lalin.fields[[
    values: ptr(LuaRTValue);
    base: i64;
    top: i64
  ]])
  bundle:struct(StackModel.WINDOW_TYPE_NAME, lalin.fields[[
    values: ptr(LuaRTValue);
    base: i64;
    count: i64
  ]])
  bundle:struct(StackModel.SEQ_TYPE_NAME, lalin.fields[[
    kind: i64;
    count: i64;
    value0: LuaRTValue;
    value1: LuaRTValue;
  ...
  bundle:struct(StackModel.VARARG_TYPE_NAME, lalin.fields[[
    kind: i64;
    values: ptr(LuaRTValue);
    count: i64;
    table_handle: i64
  ]])
  bundle:struct(CallModel.FRAME_TYPE_NAME, lalin.fields[[
  ...
  bundle:struct(ObjectModel.STRING_TYPE_NAME, lalin.fields[[
    byte_len: i64;
    hash: i64;
    numeric_kind: i64;
    numeric_i64: i64;
  ...
  bundle:struct(ObjectModel.HASH_ENTRY_TYPE_NAME, lalin.fields[[
    state: i64;
    key: LuaRTValue;
    value: LuaRTValue
  ]])
  bundle:struct(ObjectModel.TABLE_TYPE_NAME, lalin.fields[[
  ...
  bundle:struct(ObjectModel.RAW_GET_TYPE_NAME, lalin.fields[[
    hit: bool;
    value: LuaRTValue
  ]])
  bundle:struct(CDataModel.TYPE_NAME, lalin.fields[[
    data_i32: ptr(i32);
    data_i64: ptr(i64);
    data_f64: ptr(f64);
    size_bytes: i64;
...
function M.build_bundle(kernel, opts)
  ...
  if not ok then return nil, errors end
  local name = render_name(opts.name or (kernel.id and kernel.id.name) or "spongejit_quote_kernel")
  local bundle = lalin.module(name .. "_bundle")
  declare_runtime_abi(bundle)

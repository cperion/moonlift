-- lua_exec_to_lalin_cfg_lower.lua -- mechanical LuaExec semantic CFG -> LalinCFG.
--
...
local T = B.T
local RT, Exec, CC, CFG, Compile = T.LuaRT, T.LuaExec, T.CompileContract, T.LalinCFG, T.LuaCompile
...
local function type_info(exec_type)
  ...
  if cls == Exec.LalinType then return exec_type.lalin_type, { ty = exec_type.lalin_type } end
...
local function param_meta(p) local _lalin, meta = type_info(p.type); return meta end
...
local function lower_value(kernel, opts)
  ...
  if not region_ok then
    return nil, { "lua_exec_to_lalin_cfg:unsupported_semantic_region:" .. tostring(region.kind and region.kind.kind) .. ":" .. tostring(region_reason) }
  end
...
local phase = pvm.phase("spongejit_lua_exec_to_lalin_cfg_lower", function(kernel, mode, projection)
  ...
  -- hard-error for unsupported LuaExec/LalinCFG slices.
  local cfg, errors = lower_value(kernel, opts_from_mode(mode, projection))
  if cfg then return Compile.LalinLowerOk(cfg) end
  return Compile.LalinLowerReject(errors or { "lua_exec_to_lalin_cfg_lower_failed" })
end)
...
local module_phase = pvm.phase("spongejit_lua_exec_module_to_lalin_cfg_lower", function(module, kernel_name, mode, projection)
  ...
  if cfg then return Compile.LalinLowerOk(cfg) end
  return Compile.LalinLowerReject(errors or { "lua_exec_module_to_lalin_cfg_lower_failed" })
end)

local function unwrap_lalin_result(result, fallback)
  if pvm.classof(result) == Compile.LalinLowerOk then return result.kernel end
  if pvm.classof(result) == Compile.LalinLowerReject then return nil, result.errors end
  return nil, { fallback }
end
...
function M.lower(kernel, opts)
  return unwrap_lalin_result(M.lower_result(kernel, opts), "lua_exec_to_lalin_cfg_lower_no_result")
end
...
function M.lower_module(module, kernel_name, opts)
  return unwrap_lalin_result(M.lower_module_result(module, kernel_name, opts), "lua_exec_module_to_lalin_cfg_lower_no_result")
end

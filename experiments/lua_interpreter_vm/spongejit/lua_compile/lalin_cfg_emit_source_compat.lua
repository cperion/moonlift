-- lalin_cfg_emit_source_compat.lua -- compatibility/debug LalinCFG -> Lalin source renderer.
--
...
-- The primary semantic emission path must use lalin_cfg_quote_emit.lua, which
-- constructs Lalin through lalin.* typed quotes/splices.  This renderer still
-- prints LalinCFG only: it must not inspect source semantics, synthesize helper
...
local function render_type(ty)
  return tostring(ty and ty.lalin_type or ty or "void")
end
...
local function render_const(c)
  ...
  error("unsupported LalinCFG.Const reached emission: " .. tostring(c and c.kind))
end
...
local function render_place(place)
  ...
  error("unsupported LalinCFG.Place reached emission: " .. tostring(place and place.kind))
end
...
function render_value(v)
  ...
  error("unsupported LalinCFG.Value reached emission: " .. tostring(v and v.kind))
end
...
local function render_primitive(op, args)
  ...
  error("unsupported LalinCFG.PrimOp reached emission: " .. tostring(k))
end
...
local function render_expr(e)
  ...
  error("unsupported LalinCFG.Expr reached emission: " .. tostring(e and e.kind))
end
...
local function render_choice(choice)
  ...
  error("unsupported LalinCFG.Choice reached emission: " .. tostring(choice and choice.kind))
end
...
local function render_op(op, indent)
  ...
  error("unsupported LalinCFG.Op reached emission: " .. tostring(op and op.kind))
end
...
local function render_terminator(term, indent, mode)
  ...
  error("unsupported LalinCFG.Terminator reached emission: " .. tostring(term and term.kind))
end

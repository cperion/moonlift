# Lalin WebUI Blueprint

**Status:** canonical blueprint for the Lalin WebUI framework: authoring API, generated Lalin shape, runtime product/protocol tree, memory model, and public ABI.

This document folds together:

1. the Lalin WebUI `.mlua` design header, where the product graph and protocol graph are the design;
2. the final authoring decision: every tree-shaped thing is built with Lua callable tables and no parentheses, while executable leaves are already Lalin values.
...
The app author defines a component as a Lua tree. The compiler stamps it into monomorphic Lalin:
...
All executable behavior leaves are already Lalin values.
```

Lua writes the trees. Lalin supplies the typed leaves. The compiler wires the forest into native state machines.

This means:

```text
component/state/view/list/when/child = Lua tree DSL
called guards/actions/expressions    = Lalin region/expr/function values
runtime behavior                     = generated Lalin regions
...
### 4.3 Executable leaves are Lalin values

The following are not Lua callbacks. They are hosted Lalin declarations bound to Lua values.
...
Runtime behavior is never a Lua callback. Lua may create the AST at build time, but guards/actions/effects are Lalin values. If an author wants a custom behavior leaf, they write a Lalin `region`, `func`, or `expr` and place that value into the tree.

### 5.3 Curried environment pattern

Builders support environment currying like Lalin’s quote/binder style:
...
A guard is a Lalin region whose continuation names become legal branch names.
...
An action is a Lalin region. The transition must either use a conventional exit (`done`, `stored`, `spawned`) or fill its action exits explicitly.
...
bind.text { title_expr }   -- Lalin expr binding
...
struct ViewBindingSpec
    ...
    field: FieldRef
    expr_id: u32          -- compiler-private handle to Lalin expr value if source_kind=expr
    node: ViewNodeRef
...
target states exist
guard is a Lalin region value
action is a Lalin region value
...
[ ] Every executable leaf is a Lalin value, not a runtime Lua callback.

# Zyn Lalin Synth Implementation Spec

Sibling contract for `zyn_lalin_synth_headers.mlua`.

This file is the behavioral contract required before `zyn_lalin_synth_impl.mlua`
...
Lalin code owns no hidden global state. The host owns raw memory and audio/event
...
field order for corresponding Lalin products are byte-identical to the
Lalin product fields, with natural alignment and no padding beyond the

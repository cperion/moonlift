...
for i = 1, #program.cmds do
    ...
    if pvm.classof(cmd) == T.Moon2Back.CmdBinary and cmd.op == T.Moon2Back.BackVecIadd then saw_vec = true end
end
assert(saw_vec, "expected contract kernel to vectorize")

local artifact = jit_api.jit():compile(bridge.lower_program(program))
local add = ffi.cast("int32_t (*)(int32_t*, const int32_t*, const int32_t*, int32_t)", artifact:getpointer(B1.BackFuncId("add_noalias_i32")))

...
local phase_src = E.emit_phase_body(body)
...
local cached_src = E.emit_phase_module(body)
assert(cached_src:find("type type_classifyCache = struct", 1, true))
assert(cached_src:find("func type_classify_drain_one_uncached", 1, true))
assert(cached_src:find("export func type_classify", 1, true))
assert(cached_src:find("if hit.valid then", 1, true))
assert(cached_src:find("return hit.value", 1, true))

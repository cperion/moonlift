...
local xs_binding = Bind.Binding(Core.Id("arg.xs"), "xs", ptr_index_ty, Bind.BindingRoleArg(0))
local i_binding = Bind.Binding(Core.Id("control.sum.loop.i"), "i", index_ty, Bind.BindingRoleEntryBlockParam("control.sum", "loop", 1))
...
local acc_binding = Bind.Binding(Core.Id("control.sum.loop.acc"), "acc", index_ty, Bind.BindingRoleEntryBlockParam("control.sum", "loop", 2))

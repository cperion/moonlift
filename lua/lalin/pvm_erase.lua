...
function M.transform_source(src, opts)
    ...
    if need_schema or need_phase_output then
        local lines = {}
        if out:match("pvm%.") then lines[#lines + 1] = "local pvm = require(\"lalin.pvm\")" end
        if need_schema then lines[#lines + 1] = "local schema = require(\"lalin.schema_runtime\")" end
        ...
        out = out:gsub("local%s+pvm%s*=%s*require%(([\"'])lalin%.pvm%1%)", replacement, count)
        if not out:match("lalin%.schema_runtime") and need_schema then
            out = "local schema = require(\"lalin.schema_runtime\")\n" .. out
        end

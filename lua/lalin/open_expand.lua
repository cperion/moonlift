...
local function bind_context(T, opts)
    ...
    function lookup_slot_value(node, ...)
        ...
        elseif schema.isa(node, O.SlotName) then
            ...
            error("phase lalin_open_lookup_slot_value: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(self)
        ...
        elseif schema.isa(node, O.NameRefSlot) then
            ...
            error("phase lalin_open_expand_name_ref: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(self)
        ...
        elseif schema.isa(node, Ty.TypeRefSlot) then
            ...
            error("phase lalin_open_expand_type_ref: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(Ty.HandleTarget(one(expand_type_ref, self.target, env)))
            ...
            error("phase lalin_open_expand_handle_fact: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(self)
            ...
            error("phase lalin_open_expand_type: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function expand_open_set(node, ...)
        ...
        if schema.isa(node, O.OpenSet) then
            ...
            error("phase lalin_open_expand_open_set: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { ty = one(expand_type, self.ty, env) }))
        ...
        elseif schema.isa(node, Tr.ExprOpen) then
            ...
            error("phase lalin_open_expand_expr_header: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { ty = one(expand_type, self.ty, env) }))
        ...
        elseif schema.isa(node, Tr.PlaceOpen) then
            ...
            error("phase lalin_open_expand_place_header: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(self)
            ...
            error("phase lalin_open_expand_stmt_header: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(self)
            ...
            error("phase lalin_open_expand_module_header: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function expand_binding(node, ...)
        ...
        if schema.isa(node, B.Binding) then
            ...
            error("phase lalin_open_expand_binding: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return {}
            ...
            error("phase lalin_open_expand_value_ref_expr: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(self)
            ...
            error("phase lalin_open_expand_value_ref: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { base = one(expand_view, self.base, env), elem = one(expand_type, self.elem, env), stride = one(expand_expr, self.stride, env), lane = one(expand_expr, self.lane, env) }))
            ...
            error("phase lalin_open_expand_view: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { view = one(expand_view, self.view, env) }))
        ...
        elseif schema.isa(node, Tr.DomainSlotValue) then
            ...
            error("phase lalin_open_expand_domain: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { view = one(expand_view, self.view, env) }))
            ...
            error("phase lalin_open_expand_index_base: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { h = one(expand_place_header, self.h, env), base = one(expand_index_base, self.base, env), index = one(expand_expr, self.index, env) }))
        ...
        elseif schema.isa(node, Tr.PlaceSlotValue) then
            ...
            error("phase lalin_open_expand_place: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    local RegionCall = require("lalin.region_call_lowering")(T, {
    ...
    local rnf = require("lalin.region_normal_form")(T, {
    ...
    function expand_control_stmt_region(node, ...)
        ...
        if schema.isa(node, Tr.ControlStmtRegion) then
            ...
            error("phase lalin_open_expand_control_stmt_region: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function expand_control_expr_region(node, ...)
        ...
        if schema.isa(node, Tr.ControlExprRegion) then
            ...
            error("phase lalin_open_expand_control_expr_region: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { h = one(expand_expr_header, self.h, env), ty = one(expand_type, self.ty, env), addr = one(expand_expr, self.addr, env), expected = one(expand_expr, self.expected, env), replacement = one(expand_expr, self.replacement, env) }))
        ...
        elseif schema.isa(node, Tr.ExprUseExprFrag) then
            ...
            error("phase lalin_open_expand_expr: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { h = one(expand_stmt_header, self.h, env) }))
            ...
            error("phase lalin_open_expand_stmt: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function expand_func(node, ...)
        ...
        elseif schema.isa(node, Tr.FuncOpen) then
            ...
            error("phase lalin_open_expand_func: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { result = one(expand_type, self.result, env) }))
            ...
            error("phase lalin_open_expand_extern: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { open = one(expand_open_set, self.open, env), ty = one(expand_type, self.ty, env), value = one(expand_expr, self.value, env) }))
            ...
            error("phase lalin_open_expand_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { open = one(expand_open_set, self.open, env), ty = one(expand_type, self.ty, env), value = one(expand_expr, self.value, env) }))
            ...
            error("phase lalin_open_expand_static: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { facts = expand_handle_facts(self.facts, env) }))
        ...
        elseif schema.isa(node, Tr.TypeDeclOpenUnion) then
            ...
            error("phase lalin_open_expand_type_decl: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(schema.with(self, { frag = expand_expr_frag_node(self.frag, env) }))
        ...
        elseif schema.isa(node, Tr.ItemUseModuleSlot) then
            ...
            error("phase lalin_open_expand_item: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function expand_module(node, ...)
        ...
        if schema.isa(node, Tr.Module) then
            ...
            error("phase lalin_open_expand_module: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end

...
local function bind_context(T)
    ...
    local function tokens_phase(node, ...)
        ...
        return as_list(filtered)
            ...
            error("phase lalin_editor_semantic_tokens: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    local function range_tokens_phase(node, ...)
        ...
        return as_list(out)
            ...
            error("phase lalin_editor_semantic_tokens_range: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end

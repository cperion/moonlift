...
if not rawget(_G, "__lalin_value_proxy_cdef") then
    ffi.cdef[[
    typedef struct LalinValueRef {
        uint64_t session_id;
        uint32_t family_id;
        uint32_t type_id;
        uint32_t tag;
    ...
    rawset(_G, "__lalin_value_proxy_cdef", true)
end
...
local function family_for_id(id)
    ...
    if not family then error("unknown Lalin value proxy family " .. tostring(id), 3) end
    return family
end
...
function M.new_ref(family_id, opts)
    opts = opts or {}
    local ref = ffi.new("LalinValueRef[1]")
...
function Methods:to_table()
    ...
    error("Lalin value proxy family '" .. tostring(family.name) .. "' cannot materialize to a Lua table")
end
...
function MT.__newindex()
    error("Lalin value proxies are immutable", 2)
end
...
function MT.__len(self)
    ...
    error("Lalin value proxy family '" .. tostring(family.name) .. "' has no length")
end
...
function MT.__tostring(self)
    ...
    return "LalinValue(" .. tostring(family.name) .. ")"
end

...
local function assert_region_value(frag, site)
    assert(type(frag) == "table" and (frag.kind == "region_frag" or frag.lalin_quote_kind == "region_frag") and frag.name, (site or "compose") .. " expects RegionFragValue")
end
...
function Compose.new(lalin_or_session, opts)
    opts = opts or {}
    local api, session, prefix
    if type(lalin_or_session) == "table" and lalin_or_session.region_frag then
        api = lalin_or_session
        session = api.session
        prefix = opts.prefix or opts[1]
    elseif type(lalin_or_session) == "table" and lalin_or_session.api then
        session = lalin_or_session
        api = session:api()
        prefix = opts.prefix or opts[1]
    else
        error("region_compose.new expects a lalin host api or HostSession", 2)
    end
...
function Compose:validate_complete_routes(frag, routes)
    local H = self.session.T.LalinHost
    ...
    return api.region_frag(name, params, conts, function(r)
        ...
        for i = #fragments, 2, -1 do
            ...
            if not prev_exit then
                api.raise_host_issue(self.session.T.LalinHost.HostIssueRegionComposeMissingExit(fragments[i - 1].name, through))
            end
            local bparams = {}
            ...
            after[i - 1]:body_fn(function(b)
                ...
                for cname in pairs(fragments[i].conts or {}) do
                    ...
                    else api.raise_host_issue(self.session.T.LalinHost.HostIssueRegionComposeIncompleteRoute(fragments[i].name, cname)) end
                end
        ...
        r:entry("start", {}, function(b)
            ...
            for cname in pairs(fragments[1].conts or {}) do
                ...
                elseif r.conts[cname] then fills[cname] = r.conts[cname]
                elseif cname == failure and r.conts[failure] then fills[cname] = r.conts[failure]
                else api.raise_host_issue(self.session.T.LalinHost.HostIssueRegionComposeIncompleteRoute(fragments[1].name, cname)) end
            end
...
function Compose:choice(alternatives, opts)
    ...
    return api.region_frag(name, params, conts, function(r)
        ...
        for i = #alternatives, 2, -1 do
            ...
            if not fail_exit then api.raise_host_issue(self.session.T.LalinHost.HostIssueRegionComposeMissingExit(alternatives[i - 1].name, failure)) end
            local bparams = {}
            ...
            tries[i - 1]:body_fn(function(b)
                ...
                for cname in pairs(alternatives[i].conts or {}) do
                    ...
                    elseif r.conts[cname] then fills[cname] = r.conts[cname]
                    else api.raise_host_issue(self.session.T.LalinHost.HostIssueRegionComposeIncompleteRoute(alternatives[i].name, cname)) end
                end
        ...
        r:entry("start", {}, function(b)
            ...
            for cname in pairs(alternatives[1].conts or {}) do
                ...
                elseif r.conts[cname] then fills[cname] = r.conts[cname]
                else api.raise_host_issue(self.session.T.LalinHost.HostIssueRegionComposeIncompleteRoute(alternatives[1].name, cname)) end
            end

...
local function bind_context(T)
    ...
    local function prepare_rename_phase(node, ...)
        ...
        if schema.isa(node, E.PositionQuery) then
            ...
            error("phase lalin_editor_prepare_rename: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    local function rename_phase(node, ...)
        ...
        if schema.isa(node, E.RenameQuery) then
            ...
            error("phase lalin_editor_rename: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end

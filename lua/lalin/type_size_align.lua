...
local function bind_context(T)
 ...
 return single(index_layout(target))
            ...
            error("phase lalin_type_scalar_mem_layout: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function class_layout(node, ...)
        ...
        elseif schema.isa(node, Ty.TypeClassUnknown) then
            ...
            error("phase lalin_type_class_mem_layout: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end

...
local function bind_context(T)
    ...
    function import_call_target(node, ...)
        ...
        elseif schema.isa(node, O.ImportGlobalStatic) then
            ...
            error("phase moonlift_sem_import_call_target: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(closure_or_indirect(callee, fn_ty))
        ...
        elseif schema.isa(node, B.BindingClassOpenSlot) then
            ...
            error("phase moonlift_sem_binding_class_call_target: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single({ kind = "unresolved", callee = callee })
            ...
            error("phase moonlift_sem_value_ref_call_target: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single({ kind = "unresolved", callee = callee })
            ...
            error("phase moonlift_sem_call_decide: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end

-- MoonCompiler ABI boundary validation.
--
-- CodeResult is the persisted hosted-code boundary between semantic MoonTree
...
function M.Define(T)
    ...
    function api.validate_code_result(code_result, opts)
        ...
        if pvm.classof(code_result) ~= Compiler.CodeResult then
            add(issues, Compiler.CodeResultIssueWrongClass("MoonCompiler.CodeResult", class_name(code_result)))
            return Compiler.CodeResultReport(issues)
        end

        local module_ok = check_field(issues, code_result.module, "module", Code.CodeModule, "MoonCode.CodeModule")
        check_field(issues, code_result.layout_env, "layout_env", Sem.LayoutEnv, "MoonSem.LayoutEnv")

        if type(code_result.contracts) ~= "table" or pvm.classof(code_result.contracts) then
            add(issues, Compiler.CodeResultIssueInvalidField("contracts", "MoonCode.CodeFuncContractFact[]", class_name(code_result.contracts)))
        else
            for i = 1, #code_result.contracts do
                if pvm.classof(code_result.contracts[i]) ~= Code.CodeFuncContractFact then
                    add(issues, Compiler.CodeResultIssueInvalidField("contracts[" .. tostring(i) .. "]", "MoonCode.CodeFuncContractFact", class_name(code_result.contracts[i])))
                end
            end
        end
    ...
    function api.assert_valid_code_result(code_result, opts)
        ...
        error("moonlift compiler ABI CodeResult validation failed:\n" .. table.concat(messages, "\n"), 2)
    end

    api.validate = api.validate_code_result

    T._moonlift_api_cache.compiler_abi = api
    return api
end

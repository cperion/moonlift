...
function DraftStructValue:add_field(name, ty)
    if self.sealed then self.api.raise_host_issue(self.session.T.Moon2Host.HostIssueSealedMutation(self.name)) end
    assert_name(name, "struct field")
    if self.fields_by_name[name] ~= nil then self.api.raise_host_issue(self.session.T.Moon2Host.HostIssueDuplicateField(self.name, name)) end
...
function DraftStructValue:seal()
    if self.sealed then self.api.raise_host_issue(self.session.T.Moon2Host.HostIssueAlreadySealed(self.name)) end
    local decls = {}
...
function M.Install(api, session)
    local T = session.T
    local C, Ty, Tr = T.Moon2Core, T.Moon2Type, T.Moon2Tree
    ...
    local function normalize_fields(fields)
        ...
        if dup ~= nil then api.raise_host_issue(session.T.Moon2Host.HostIssueDuplicateField("<type>", tostring(dup))) end
        return out, by_name
    end

...
function M.Install(api, session)
    local T = session.T
    local B, Tr = T.Moon2Bind, T.Moon2Tree
    ...
    function PlaceValue:field(name, ty)
        ...
        return place_value(Tr.PlaceField(Tr.PlaceSurface, self.place, T.Moon2Sem.FieldByName(name, ft.ty)), ft, (self.source_hint or "<place>") .. "." .. name)
    end

...
function M.Define(T)
    ...
    local function eval_expr(expr, const_env, local_env)
        return erased.one(expr_const_class(expr, const_env, local_env))
    end
    ...
    local function eval_stmts(stmts, const_env, local_env)
        ...
        for i = 1, #stmts do
            local result = erased.one(stmt_const_result(stmts[i], const_env, current))
 ...
 return erased.once(self.ty)
            ...
            error("erased phase moonlift_sem_const_expr_type: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return erased.once(yes(Sem.ConstInt(ty, self.raw)))
            end)(node, ...)
        elseif schema.isa(node, C.LitFloat) then
            return (function(self, ty)
 return erased.once(yes(Sem.ConstFloat(ty, self.raw)))
            end)(node, ...)
        elseif schema.isa(node, C.LitBool) then
            return (function(self)
 return erased.once(yes(Sem.ConstBool(self.value)))
            end)(node, ...)
        elseif schema.isa(node, C.LitString) then
            return (function()
 return erased.once(no())
            end)(node, ...)
        elseif schema.isa(node, C.LitNil) then
            return (function(_, ty)
 return erased.once(yes(Sem.ConstNil(ty)))
            end)(node, ...)
        else
            error("erased phase moonlift_sem_literal_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function unary_const(node, ...)
        ...
        if schema.isa(node, C.UnaryNeg) then
            ...
            if raw == nil then return erased.once(no()) end
            return erased.once(yes(Sem.ConstInt(ty, number_string(-tonumber(raw)))))
            end)(node, ...)
        elseif schema.isa(node, C.UnaryNot) then
            return (function(_, value)

            local b = bool_value(value)
            if b == nil then return erased.once(no()) end
            return erased.once(yes(Sem.ConstBool(not b)))
            end)(node, ...)
        elseif schema.isa(node, C.UnaryBitNot) then
            ...
            local raw = int_raw(value)
            if raw == nil then return erased.once(no()) end
            return erased.once(yes(Sem.ConstInt(ty, tostring(bit.bnot(tonumber(raw))))) )
            end)(node, ...)
        else
            error("erased phase moonlift_sem_unary_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, number_string(x + y))))
            end)(node, ...)
 ...
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, number_string(x * y))))
        ...
        elseif schema.isa(node, C.BinDiv) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y or y == 0 then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, number_string(math.floor(x / y)))))
            end)(node, ...)
        elseif schema.isa(node, C.BinRem) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y or y == 0 then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, number_string(x % y))))
            end)(node, ...)
        elseif schema.isa(node, C.BinBitAnd) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, tostring(bit.band(x, y)))))
            end)(node, ...)
        elseif schema.isa(node, C.BinBitOr) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, tostring(bit.bor(x, y)))))
            end)(node, ...)
        elseif schema.isa(node, C.BinBitXor) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, tostring(bit.bxor(x, y)))))
            end)(node, ...)
        elseif schema.isa(node, C.BinShl) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, tostring(bit.lshift(x, y)))))
            end)(node, ...)
        elseif schema.isa(node, C.BinLShr) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, tostring(bit.rshift(x, y)))))
            end)(node, ...)
        elseif schema.isa(node, C.BinAShr) then
            return (function(_, a, b, ty)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstInt(ty, tostring(bit.arshift(x, y)))))
            end)(node, ...)
        else
            error("erased phase moonlift_sem_binary_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return erased.once(yes(Sem.ConstBool(same_const(a, b))))
            end)(node, ...)
        elseif schema.isa(node, C.CmpNe) then
            return (function(_, a, b)
 return erased.once(yes(Sem.ConstBool(not same_const(a, b))))
            end)(node, ...)
        elseif schema.isa(node, C.CmpLt) then
            return (function(_, a, b)
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstBool(x < y)))
            end)(node, ...)
 ...
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return erased.once(no()) end; return erased.once(yes(Sem.ConstBool(x >= y)))
            ...
            error("erased phase moonlift_sem_compare_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function logic_const(node, ...)
        ...
        if schema.isa(node, C.LogicAnd) then
            return (function(_, a, b)
 local x, y = bool_value(a), bool_value(b); if x == nil or y == nil then return erased.once(no()) end; return erased.once(yes(Sem.ConstBool(x and y)))
            end)(node, ...)
        elseif schema.isa(node, C.LogicOr) then
            return (function(_, a, b)
 local x, y = bool_value(a), bool_value(b); if x == nil or y == nil then return erased.once(no()) end; return erased.once(yes(Sem.ConstBool(x or y)))
            end)(node, ...)
        else
            error("erased phase moonlift_sem_logic_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function value_ref_const(node, ...)
        ...
        if schema.isa(node, B.ValueRefBinding) then
            ...
            if local_value ~= nil then return erased.once(yes(local_value)) end
 ...
 return erased.once(no())
        ...
        elseif schema.isa(node, B.ValueRefHole) then
            return (function()
 return erased.once(nil)
            end)(node, ...)
        else
            error("erased phase moonlift_sem_value_ref_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return value_ref_const(self.ref, const_env, local_env)
        ...
        elseif schema.isa(node, Tr.ExprUnary) then
            ...
            if v == nil then return erased.once(no()) end
            local tys = expr_type(self.h)
        ...
        elseif schema.isa(node, Tr.ExprBinary) then
            ...
            if a == nil or b == nil then return erased.once(no()) end
            local tys = expr_type(self.h)
        ...
        elseif schema.isa(node, Tr.ExprCast) then
            ...
            local v = eval_value(self.value, const_env, local_env)
            if v == nil then return erased.once(no()) end
            if schema.classof(v) == Sem.ConstInt then return erased.once(yes(Sem.ConstInt(self.ty, v.raw))) end
            if schema.classof(v) == Sem.ConstFloat then return erased.once(yes(Sem.ConstFloat(self.ty, v.raw))) end
            if schema.classof(v) == Sem.ConstNil then return erased.once(yes(Sem.ConstNil(self.ty))) end
            return erased.once(yes(v))
            end)(node, ...)
 ...
 return erased.once(no())
        ...
        elseif schema.isa(node, Tr.ExprField) then
            ...
            if base == nil or schema.classof(base) ~= Sem.ConstAgg then return erased.once(no()) end
        ...
        elseif schema.isa(node, Tr.ExprIndex) then
            ...
            local base = nil
            if schema.classof(self.base) == Tr.IndexBaseView then return erased.once(no()) end
            ...
            if base == nil or schema.classof(base) ~= Sem.ConstArray or n == nil then return erased.once(no()) end
        ...
        elseif schema.isa(node, Tr.ExprAgg) then
            ...
            return erased.once(yes(Sem.ConstAgg(self.ty, fields)))
            end)(node, ...)
        elseif schema.isa(node, Tr.ExprArray) then
            ...
            return erased.once(yes(Sem.ConstArray(self.elem_ty, elems)))
            end)(node, ...)
        ...
        elseif schema.isa(node, Tr.ExprSwitch) then
            ...
            if value == nil then return erased.once(no()) end
 ...
 return erased.once(no())
        ...
        elseif schema.isa(node, Tr.ExprBlock) then
            ...
            if result.kind ~= "falls_through" then return erased.once(no()) end
 ...
 return erased.once(no())
            ...
            error("erased phase moonlift_sem_expr_const_class: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function stmt_const_result(node, ...)
        ...
        if schema.isa(node, Tr.StmtLet) then
            ...
            if v == nil then return erased.once({ kind = "falls_through", env = local_env }) end
            return erased.once({ kind = "falls_through", env = local_put(local_env, self.binding, v) })
            end)(node, ...)
 ...
 return erased.once({ kind = "falls_through", env = local_env })
            end)(node, ...)
 ...
 return erased.once({ kind = "falls_through", env = local_env })
        ...
        elseif schema.isa(node, Tr.StmtExpr) then
            return (function(self, const_env, local_env)
 eval_expr(self.expr, const_env, local_env); return erased.once({ kind = "falls_through", env = local_env })
            end)(node, ...)
 ...
 return erased.once({ kind = "falls_through", env = local_env })
        ...
        elseif schema.isa(node, Tr.StmtIf) then
            ...
            local cond = eval_value(self.cond, const_env, local_env)
            local b = cond and bool_value(cond)
            if b == nil then return erased.once({ kind = "falls_through", env = local_env }) end
            if b then return erased.once(eval_stmts(self.then_body, const_env, local_env)) end
            return erased.once(eval_stmts(self.else_body, const_env, local_env))
            end)(node, ...)
 ...
 return erased.once({ kind = "falls_through", env = local_env })
        ...
        elseif schema.isa(node, Tr.StmtReturnVoid) then
            return (function(_, _, local_env)
 return erased.once({ kind = "return_void", env = local_env })
            end)(node, ...)
        elseif schema.isa(node, Tr.StmtReturnValue) then
            return (function(self, const_env, local_env)

            local v = eval_value(self.value, const_env, local_env)
            if v == nil then return erased.once({ kind = "return_void", env = local_env }) end
            return erased.once({ kind = "return_value", env = local_env, value = v })
            end)(node, ...)
        elseif schema.isa(node, Tr.StmtJump) then
            return (function(self, _, local_env)
 return erased.once({ kind = "jump", env = local_env, target = self.target.name })
            end)(node, ...)
 ...
 return erased.once({ kind = "falls_through", env = local_env })
        ...
        elseif schema.isa(node, Tr.StmtYieldVoid) then
            return (function(_, _, local_env)
 return erased.once({ kind = "yield_void", env = local_env })
            end)(node, ...)
        elseif schema.isa(node, Tr.StmtYieldValue) then
            return (function(self, const_env, local_env)

            local v = eval_value(self.value, const_env, local_env)
            if v == nil then return erased.once({ kind = "yield_void", env = local_env }) end
            return erased.once({ kind = "yield_value", env = local_env, value = v })
            end)(node, ...)
 ...
 return erased.once({ kind = "falls_through", env = local_env })
            ...
            error("erased phase moonlift_sem_stmt_const_result: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    return {
        ...
        expr = function(expr, const_env, local_env) return erased.one(expr_const_class(expr, const_env or empty_const_env(), local_env or empty_local_env())) end,
        value = function(expr, const_env, local_env) return const_value(erased.one(expr_const_class(expr, const_env or empty_const_env(), local_env or empty_local_env()))) end,

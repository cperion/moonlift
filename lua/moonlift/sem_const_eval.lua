...
local function bind_context(T)
 ...
 return single(self.ty)
            ...
            error("phase moonlift_sem_const_expr_type: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(yes(Sem.ConstNil(ty)))
            ...
            error("phase moonlift_sem_literal_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function unary_const(node, ...)
        ...
        elseif schema.isa(node, C.UnaryBitNot) then
            ...
            error("phase moonlift_sem_unary_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return single(no()) end; return single(yes(Sem.ConstInt(ty, tostring(bit.arshift(x, y)))))
            ...
            error("phase moonlift_sem_binary_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 local x, y = tonumber(int_raw(a)), tonumber(int_raw(b)); if not x or not y then return single(no()) end; return single(yes(Sem.ConstBool(x >= y)))
            ...
            error("phase moonlift_sem_compare_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 local x, y = bool_value(a), bool_value(b); if x == nil or y == nil then return single(no()) end; return single(yes(Sem.ConstBool(x or y)))
            ...
            error("phase moonlift_sem_logic_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(nil)
            ...
            error("phase moonlift_sem_value_ref_const: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single(no())
            ...
            error("phase moonlift_sem_expr_const_class: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
 ...
 return single({ kind = "falls_through", env = local_env })
            ...
            error("phase moonlift_sem_stmt_const_result: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end

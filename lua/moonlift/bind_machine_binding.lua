...
local function bind_context(T)
    ...
    function decision_machine_binding(node, ...)
        ...
        if schema.isa(node, B.ResidenceDecision) then
            ...
            error("phase moonlift_bind_decision_machine_binding: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end
    ...
    function plan_machine_bindings(node, ...)
        ...
        if schema.isa(node, B.ResidencePlan) then
            ...
            error("phase moonlift_bind_plan_machine_bindings: no handler for " .. tostring(cls and cls.kind or type(node)), 2)
        end
    end

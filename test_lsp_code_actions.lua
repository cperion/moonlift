...
local uri = "file:///tmp/actions.mlua"
...
local input = table.concat({
    ...
    frame({ jsonrpc = "2.0", id = 2, method = "textDocument/codeAction", params = { textDocument = { uri = uri }, range = { start = { line = 0, character = 0 }, ["end"] = { line = 3, character = 0 } }, context = { diagnostics = {} } } }),
...
local actions = by_id[2].result
assert(#actions == 2)
assert(actions[1].kind == "quickfix")
assert(actions[1].diagnostics[1].code == "host.bareBoolBoundary")
local edits = actions[1].edit.changes[uri]
assert(#edits == 1)
assert(edits[1].newText == "bool32")
assert(actions[2].edit.changes[uri][1].newText == "bool8")

print("moonlift lsp code actions ok")
